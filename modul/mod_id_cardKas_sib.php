<?php
	   echo "<br><a href=?module=idcardKasSib&act=cari&huruf=A>A</A> |	<a href=?module=idcardKasSib&act=cari&huruf=B>B</A> |	<a href=?module=idcardKasSib&act=cari&huruf=C>C</A> |	<a href=?module=idcardKasSib&act=cari&huruf=D>D</A> |	<a href=?module=idcardKasSib&act=cari&huruf=E>E</A> |	<a href=?module=idcardKasSib&act=cari&huruf=F>F</A> |	<a href=?module=idcardKasSib&act=cari&huruf=G>G</A> |	<a href=?module=idcardKasSib&act=cari&huruf=H>H</A> |	<a href=?module=idcardKasSib&act=cari&huruf=I>I</A> |	<a href=?module=idcardKasSib&act=cari&huruf=J>J</A> |	<a href=?module=idcardKasSib&act=cari&huruf=K>K</A> |	<a href=?module=idcardKasSib&act=cari&huruf=L>L</A> |	<a href=?module=idcardKasSib&act=cari&huruf=M>M</A> |	<a href=?module=idcardKasSib&act=cari&huruf=N>N</A> |	<a href=?module=idcardKasSib&act=cari&huruf=O>O</A> |	<a href=?module=idcardKasSib&act=cari&huruf=P>P</A> |	<a href=?module=idcardKasSib&act=cari&huruf=Q>Q</A> |	<a href=?module=idcardKasSib&act=cari&huruf=R>R</A> |	<a href=?module=idcardKasSib&act=cari&huruf=S>S</A> |	<a href=?module=idcardKasSib&act=cari&huruf=T>T</A> |	<a href=?module=idcardKasSib&act=cari&huruf=U>U</A> |	<a href=?module=idcardKasSib&act=cari&huruf=V>V</A> |	<a href=?module=idcardKasSib&act=cari&huruf=W>W</A> |	<a href=?module=idcardKasSib&act=cari&huruf=X>X</A> |	<a href=?module=idcardKasSib&act=cari&huruf=Y>Y</A> |	<a href=?module=idcardKasSib&act=cari&huruf=Z>Z</A>";
...
switch($_GET[act]){
  default:		    
		...
		echo "<h2>Approval ID CARD Sibling Kasie <br>$negaranya</h2>
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='idcardKasSib'>
...
<a href=?module=idcardKasSib>ALL</a> | <a href=?module=idcardKasSib&status=1 style=\"color : #B1BF19\">WAITING</a> | <a href=?module=idcardKasSib&status=2 style=\"color : green\">APPROVED</a> | 
	<a href=?module=idcardKasSib&status=0 style=\"color : #800000\">REJECTED</a> <BR>

		<form method=POST enctype='multipart/form-data' action='./aksi_id_cardK_sib.php?module=idcardKasSib&act=update'>
          <table width=100%>
	...
	if (isset($_GET[namadiplomat])){
		...
		$tampil=mysql_query("select a.NM_JNS_CETAK_KARTU,a.ID_CETAK_S,a.ID_CARD,a.ID_DIPLOMAT,a.NM_DIPLOMAT,c.ID_SIBLING,c.NM_SIBLING,c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_CARD,'%d.%m.%Y') as TGL_AWAL_CARD,  date_format(a.TGL_AKHIR_CARD,'%d.%m.%Y') as TGL_AKHIR_CARD, a.ST_KARTU,a.ST_KARTU_K,a.ST_KARTU_KAS  from v_id_card_s a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING   where c.NM_DIPLOMAT like '%".$_GET[namadiplomat]."%' and c.NEGARA like '%$neg%' and  a.ST_KARTU_KAS =  ".$_GET[status]." ORDER BY a.ID_CETAK_S desc limit $posisi,$batas");

	}
	else
    {

	$sql="select a.NM_JNS_CETAK_KARTU,a.ID_CETAK_S,a.ID_CARD,a.ID_DIPLOMAT,a.NM_DIPLOMAT,c.ID_SIBLING,c.NM_SIBLING,c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_CARD,'%d.%m.%Y') as TGL_AWAL_CARD,  date_format(a.TGL_AKHIR_CARD,'%d.%m.%Y') as TGL_AKHIR_CARD, a.ST_KARTU,a.ST_KARTU_K,a.ST_KARTU_KAS  from v_id_card_s a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING  where c.NEGARA like '%$neg%'  and  a.ST_KARTU_KAS =  ".$_GET[status]." ORDER BY a.ID_CETAK_S desc limit $posisi,$batas ";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				echo "<tr>
						<td></td>
						<td></td>
						<td><input type='button' name='updateStatus' onClick=updateAction($r[ID_CETAK_S],$noP,'Kas','4'); value='Submit'></td>
					  </tr>";
...
*/				echo "  
	...
	if (isset($_GET[status])){
		if (isset($_GET[namadiplomat]))
		{
			...
			$jmldata =mysql_num_rows(mysql_query("select a.ID_CETAK_S  from v_id_card_s a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING  where  (c.negara like '".$neg."%' and c.NM_DIPLOMAT like '%".$_GET[namadiplomat]."%') and  a.ST_KARTU_Kas =  ".$_GET[status]));

		
		}else{

			$jmldata =mysql_num_rows(mysql_query("select a.ID_CETAK_S  from v_id_card_s a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING  where  (c.negara like '".$neg."%') and  a.ST_KARTU_Kas =  ".$_GET[status]));
	...
	if (isset($_GET[status])){
	$ilink = "?module=idcardKasSib&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]&status=$_GET[status]"; }
	else
	{$ilink = "?module=idcardKasSib&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]"; }
  ...
  case "cari":
    ...
    while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				&nbsp <a href=?module=idcardKasSib&negara=$r[NEGARA]>$r[NEGARA] </a></td><td>";
