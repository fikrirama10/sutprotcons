  ...
  default:
		...
		echo "<link rel='stylesheet' href='../config/jquery-ui-1.12.1.custom/themes/base/jquery-ui.css'"
			...
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='statistik_pwk'>
...
. "<script>
 ...
 if(document.getElementById('negara').value == '' && document.getElementById('perwakilan').value == '' && document.getElementById('tgl_cr_aju1').value == '' && document.getElementById('tgl_cr_aju2').value == '' && document.getElementById('setuju').checked==false && document.getElementById('tolak').checked==false){
  ...
  window.location='report.php?go=statistik_pwk&negara='+a+'&perwakilan='+b+'&tgl_cr_aju1='+c+'&tgl_cr_aju2='+d+'&setuju='+f+'&tolak='+g;
}
}
 </script>";
 //break;      
...
//case "cari";
 ...
 echo"<table width=100%>
          <tr><th width=2%>no</th><th width=21%>KANTOR</th><th width=21%>NEGARA</th><th width=5%>STATUS</th><th width=10%>SPIRIT(1)</th><th width=10%>ANGGUR(2)</th><th width=10%>ROKOK(3)</th><th width=5%>1+2</th><th width=10%>JML (1+2+3)</th></tr>";
    ...
    echo"Hasil pencarian berdasarkan : $tgl_pengajuan_rpt $negara_rpt $knt_perwakilan $setuju_tolak";
		$sql="SELECT NM_KNT_PERWAKILAN, NEGARA, status_pengajuan, sum(jumlah_pengajuan_spirit) as spirit, sum(jumlah_pengajuan_anggur) as anggur, sum(jumlah_pengajuan_rokok) as rokok,sum(jumlah_pengajuan_spirit + jumlah_pengajuan_anggur) as jml_spr_angr,
SUM(jumlah_pengajuan_spirit + jumlah_pengajuan_anggur + jumlah_pengajuan_rokok) as total   
FROM v_report_miras_kantor where status_pengajuan is not NULL $w1 $w2 $w3 $w5
GROUP BY NM_KNT_PERWAKILAN, NEGARA, status_pengajuan
...
//echo $sql;
    ...
    echo "</table>";
		...
		$jmldata =mysql_num_rows(mysql_query("SELECT NM_KNT_PERWAKILAN, NEGARA, status_pengajuan, sum(jumlah_pengajuan_spirit) as spirit, sum(jumlah_pengajuan_anggur) as anggur, sum(jumlah_pengajuan_rokok) as rokok,sum(jumlah_pengajuan_spirit + jumlah_pengajuan_anggur) as jml_spr_angr,
SUM(jumlah_pengajuan_spirit + jumlah_pengajuan_anggur + jumlah_pengajuan_rokok) as total   
FROM v_report_miras_kantor where status_pengajuan is not NULL $w1 $w2 $w3
GROUP BY NM_KNT_PERWAKILAN, NEGARA, status_pengajuan
order by total DESC"));
	
	$jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
        
   $ilink = "?module=statistik_pwk&tgl_cr_aju1=$_GET[tgl_cr_aju1]&tgl_cr_aju2=$_GET[tgl_cr_aju2]&perwakilan=$_GET[perwakilan]&negara=$_GET[negara]&setuju=$_GET[setuju]&tolak=$_GET[tolak]"; 
