...
elseif ($module=='negara' AND $act=='update' AND isset($_POST[idt])){
  ...
  $nama_file_unik = $acak.$nama_file;
   ...
   $negara = $_POST[negara];
	...
	$KD_REGIONAL = $_POST[KD_REGIONAL];
	$NM_REGIONAL = $_POST[NM_REGIONAL];
	$NM_STATES = $_POST[NM_STATES];
	$NM_RESMI = mysql_real_escape_string($_POST[NM_RESMI]);

  // Apabila gambar tidak diganti
  if (empty($lokasi_file)){


	$update_ngr=mysql_query("update m_negara set NEGARA = '$negara',
	...
	if($update_ngr == 1){
		...
		echo"<script type='text/javascript'>
                        alert('Berhasil!!, Update Data Negara');
						document.location.href='./deplu.php?module=negara';
                    </script>";
         }else{
			 echo"<script type='text/javascript'>
                        alert('GAGAL!! Update Data Negara');
						document.location.href='./deplu.php?module=negara';
                    </script>";
		 }
	//header('location: ./deplu.php?module='.$module);
