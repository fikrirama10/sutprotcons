...
if ($TGL_AKHIR_PERMIT < $TGL_AWAL_PERMIT ){
  ...
  if (!empty($lokasi_file_setneg)){
    // Apabila tipe gambar bukan jpeg akan tampil peringatan
      if ($tipe_file_setneg != "image/jpeg" AND $tipe_file_setneg != "image/pjpeg"){
  ...
  if (!empty($lokasi_file_nota_diplomatik)){
    // Apabila tipe gambar bukan jpeg akan tampil peringatan
      if ($tipe_file_nota_diplomatik != "image/jpeg" AND $tipe_file_nota_diplomatik != "image/pjpeg"){
  ...
  if (!empty($lokasi_file_keppri_legal)){
	...
	tujuan,masa_tugas,verifikator,jabatan_verifikator,posisi,
	legalisator,jabatan_legalisator,no_nota_diplomatik
	$field_setneg $field_notadinas $field_pengganti
	,catatan,created_date,status_permohonan) 
	values 
	('$no_konsep','$nobrafaks_otvis',$pwk_ri,'$nama_otvis','$paspor_otvis',$jns_paspor,
	'$tujuan_otvis',$masatugas_otvis,'$verifikator',
	'$jab_ver',$posisi_otvis,'$legalisator','$jab_legal','$no_notadiplo'
	$file_setneg $file_notadinas $isi_pengganti
	,'$catatan','$created_date',$status_mhn)";
	//print_r($sql);exit;
	if(isset($_POST['simpan'])){
		 if(mysql_query($sql))
		 {
			...
			/* function sendmail_pwk_notif($email,$idt,$pwk_ri,$nama_otvis,$paspor_otvis,
			...
			sendmail_pwk_notif('hartarto.anugerah@kemlu.go.id',$idt,$pwk_ri,$nama_otvis,$paspor_otvis,$jns_paspor,$anggota_family,$tujuan_otvis
