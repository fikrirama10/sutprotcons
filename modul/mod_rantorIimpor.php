<?php
	   echo "<br><b><a href=?module=rantorIimpor&act=cari&huruf=A>A</A> |	<a href=?module=rantorIimpor&act=cari&huruf=B>B</A> |	<a href=?module=rantorIimpor&act=cari&huruf=C>C</A> |	<a href=?module=rantorIimpor&act=cari&huruf=D>D</A> |	<a href=?module=rantorIimpor&act=cari&huruf=E>E</A> |	<a href=?module=rantorIimpor&act=cari&huruf=F>F</A> |	<a href=?module=rantorIimpor&act=cari&huruf=G>G</A> |	<a href=?module=rantorIimpor&act=cari&huruf=H>H</A> |	<a href=?module=rantorIimpor&act=cari&huruf=I>I</A> |	<a href=?module=rantorIimpor&act=cari&huruf=J>J</A> |	<a href=?module=rantorIimpor&act=cari&huruf=K>K</A> |	<a href=?module=rantorIimpor&act=cari&huruf=L>L</A> |	<a href=?module=rantorIimpor&act=cari&huruf=M>M</A> |	<a href=?module=rantorIimpor&act=cari&huruf=N>N</A> |	<a href=?module=rantorIimpor&act=cari&huruf=O>O</A> |	<a href=?module=rantorIimpor&act=cari&huruf=P>P</A> |	<a href=?module=rantorIimpor&act=cari&huruf=Q>Q</A> |	<a href=?module=rantorIimpor&act=cari&huruf=R>R</A> |	<a href=?module=rantorIimpor&act=cari&huruf=S>S</A> |	<a href=?module=rantorIimpor&act=cari&huruf=T>T</A> |	<a href=?module=rantorIimpor&act=cari&huruf=U>U</A> |	<a href=?module=rantorIimpor&act=cari&huruf=V>V</A> |	<a href=?module=rantorIimpor&act=cari&huruf=W>W</A> |	<a href=?module=rantorIimpor&act=cari&huruf=X>X</A> |	<a href=?module=rantorIimpor&act=cari&huruf=Y>Y</A> |	<a href=?module=rantorIimpor&act=cari&huruf=Z>Z</A> </b>";
...
switch($_GET[act]){
  default:		    
		...
		echo "<h2>Import Rantor Individu<br>Pilih Diplomat - $negaranya</h2>
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='rantorIimpor'>
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				<td>$r[PEKERJAAN]</td>		
				<td>$r[TGL_TIBA]</td>		
				<td><a href=?module=rantorIimpor&act=lihat_rantorIimpor&idt=$r[ID_DIPLOMAT]&negara=$_GET[negara]>Rantor Import</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
   ...
   $ilink = "?module=rantorIimpor&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]"; 
    $linkHalaman = $p->navHalaman($ilink,$_GET[halaman], $jmlhalaman);

    echo "<div id=paging>$linkHalaman</div><br>";
    break;

  case "lihat_rantorIimpor":
	$idt = $_GET[idt];
	...
	echo " <input type=button value='Tambah' onclick=location.href='?module=rantorIimpor&act=tambah_rantorIimpor&idt=$idt&negara=$_GET[negara]'>
			<table width=100%>
          <tr><th  width=30>no</th><th>Tanggal Izin Impor</th><th >No Izin</th><th width=90>Deskripsi</th><th width=90>No Polisi</th><th width=90>STATUS DIREKTUR</th><th width=90>STATUS KASUBDIT</th> <th width=60>AKSI</th></tr>"; 

    $p      = new Paging;
    $batas  = 200;
    $posisi = $p->cariPosisi($batas);

    $tampil=mysql_query("select ID_PENGGUNAA_FAS,ID_JNS_FASILITAS,ID_DIPLOMAT,ID_KNT_PERWAKILAN,DATE_FORMAT(TGL_IZIN_IMPOR,'%d %M %Y') AS TGL_IZIN_IMPOR,DESKRIPSI,NO_IZIN_IMPOR,QTY,ST_PERSETUJUAN,ST_PERSETUJUAN_K,NO_POLISI,TAHUN,MEREK,NO_MESIN from penggunaan_fasilitas  where ID_DIPLOMAT = $idt and (ID_JNS_FASILITAS=2) and (NO_IZIN_IMPOR is not null) order by ID_PENGGUNAA_FAS ");

	$no = $posisi+1;
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td>$no</td>
                <td>$r[TGL_IZIN_IMPOR]</td>
				<td>$r[NO_IZIN_IMPOR]</td>
				<td>$r[DESKRIPSI]</td>
				<td>$r[NO_POLISI]</td>
				<td align =center>";
	...
	if ($r[ST_PERSETUJUAN_K] == 2){
		...
		echo "</td>
				<td><a href=?module=rantorIimpor&act=edit_rantorIimpor&idt=$r[ID_PENGGUNAA_FAS]&idd=$idt&negara=$_GET[negara]>Edit</a> | 
		            <a href=./aksi_rantorIimpor.php?module=rantorIimpor&act=hapus&idt=$r[ID_PENGGUNAA_FAS]&idd=$idt&negara=$_GET[negara] onClick=\"return confirm('Apakah Anda benar-benar akan menghapus Rantor $r[DESKRIPSI]?')\">Hapus</a></td>
				</tr>";
 ...
 case "tambah_rantorIimpor":
	$idt = $_GET[idt];
 $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idt  ");
	$r    = mysql_fetch_array($input);
	
	 echo "<h2 >Import Rantor Individu - Tambah</h2>";
	 echo "	  <table width=100%>
          <tr><td  width=160>Asal Negara</td>  <td > : ";
            $tampil=mysql_query("SELECT ID_NEGARA,NEGARA  FROM m_negara where ID_NEGARA = $r[ID_NEGARA]");
            $w=mysql_fetch_array($tampil);
	...
	echo "<form method=POST enctype='multipart/form-data' action='./aksi_rantorIimpor.php?module=rantorIimpor&act=input&negara=$_GET[negara]'>
          <input type=hidden name=ID_DIPLOMAT value='$r[ID_DIPLOMAT]'>
     	  <input type=hidden name=ID_KNT_PERWAKILAN value='$r[ID_KNT_PERWAKILAN]'>
		  <table width=100%>		  		 
		<th colspan=2 align = left>Import Rantor</th>	  
		<tr><td  width=120>Tanggal Persetujuan</td>  <td > 
          <DIV id=\"tgl\"> <script>DateInput('TGL_IZIN_IMPOR', true, 'YYYY-MM-DD')</script></div> </td></tr>
 ...
 case "edit_rantorIimpor":
   ...
   $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idd  ");
	$r    = mysql_fetch_array($input);
	 ...
	 echo "<h2 >Import Rantor Individu - Edit</h2>";
	echo "	  <table width=100%>
          <tr><td  width=160>Asal Negara</td>  <td > : ";
            $tampil=mysql_query("SELECT ID_NEGARA,NEGARA  FROM m_negara where ID_NEGARA = $r[ID_NEGARA]");
            $w=mysql_fetch_array($tampil);
...
NO_IZIN_JUAL,TGL_IZIN_JUAL,ALASAN_PENJUALAN,REKOMENDASI_BENGKEL,NAMA_PEMBELI,ALAMAT_PEMBELI,NO_KTP_PEMBELI from penggunaan_fasilitas where ID_PENGGUNAA_FAS = $idt ");   
	...
	echo "<form method=POST enctype='multipart/form-data' action='./aksi_rantorIimpor.php?module=rantorIimpor&act=update&idt=$idt&negara=$_GET[negara]'>
          <input type=hidden name=ID_DIPLOMAT value='$r[ID_DIPLOMAT]'>
     	  <input type=hidden name=ID_KNT_PERWAKILAN value='$r[ID_KNT_PERWAKILAN]'>
		  <table width=100%>		  		 
		<th colspan=2 align = left>Import Rantor</th>	  
		<tr><td  width=120>Tanggal Persetujuan</td>  <td > 
          <DIV id=\"tgl\"> <script>DateInput('TGL_IZIN_IMPOR', true, 'YYYY-MM-DD','$r[TGL_IZIN_IMPOR]')</script></div> </td></tr>
 ...
 break;
  case "cari":
	...
	echo "<h2>Import Rantor Individu - Pilih Negara</h2>
	<table width=100%>
    ...
    while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				&nbsp <a href=?module=rantorIimpor&negara=$r[NEGARA]>$r[NEGARA] </a></td><td>";
