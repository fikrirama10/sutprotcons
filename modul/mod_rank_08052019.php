  ...
  default:
    ...
    echo "	<tr><td>$no</td>
			...
			<td><a href=?module=rank&act=editrank&idt=$r[ID_RANK]>Edit</a> | 
				<a href=./aksi_rank.php?module=rank&act=hapus&idt=$r[ID_RANK] onClick=\"return confirm('Apakah Anda benar-benar akan menghapus $r[NM_RANK] ?')\">Hapus</a></td></tr>";
      $no++;
    }
    echo "</table>";
    ...
    $r    = mysql_fetch_array($edit);
 	echo "<h2>Edit Rank</h2>
         ...
         <input type=hidden name=idt value='$r[ID_RANK]'>
         	<table width=90%>
		  	<tr><td width=120>Nama Rank</td >  <td > : <input type=text name='NM_RANK' value='$r[NM_RANK]' size=50></td></tr>
		  	<tr><td >Kode Layanan</td >  <td > : <input type=text name='KODE_LAYANAN' value='$r[KODE_LAYANAN]' size=50></td></tr>
		  	<tr><td >Keterangan</td >  <td > : <select name='KET' >";
