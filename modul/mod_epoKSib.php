<?php
	    echo "<br><a href=?module=epoKSib&act=cari&huruf=A>A</A> |	<a href=?module=epoKSib&act=cari&huruf=B>B</A> |	<a href=?module=epoKSib&act=cari&huruf=C>C</A> |	<a href=?module=epoKSib&act=cari&huruf=D>D</A> |	<a href=?module=epoKSib&act=cari&huruf=E>E</A> |	<a href=?module=epoKSib&act=cari&huruf=F>F</A> |	<a href=?module=epoKSib&act=cari&huruf=G>G</A> |	<a href=?module=epoKSib&act=cari&huruf=H>H</A> |	<a href=?module=epoKSib&act=cari&huruf=I>I</A> |	<a href=?module=epoKSib&act=cari&huruf=J>J</A> |	<a href=?module=epoKSib&act=cari&huruf=K>K</A> |	<a href=?module=epoKSib&act=cari&huruf=L>L</A> |	<a href=?module=epoKSib&act=cari&huruf=M>M</A> |	<a href=?module=epoKSib&act=cari&huruf=N>N</A> |	<a href=?module=epoKSib&act=cari&huruf=O>O</A> |	<a href=?module=epoKSib&act=cari&huruf=P>P</A> |	<a href=?module=epoKSib&act=cari&huruf=Q>Q</A> |	<a href=?module=epoKSib&act=cari&huruf=R>R</A> |	<a href=?module=epoKSib&act=cari&huruf=S>S</A> |	<a href=?module=epoKSib&act=cari&huruf=T>T</A> |	<a href=?module=epoKSib&act=cari&huruf=U>U</A> |	<a href=?module=epoKSib&act=cari&huruf=V>V</A> |	<a href=?module=epoKSib&act=cari&huruf=W>W</A> |	<a href=?module=epoKSib&act=cari&huruf=X>X</A> |	<a href=?module=epoKSib&act=cari&huruf=Y>Y</A> |	<a href=?module=epoKSib&act=cari&huruf=Z>Z</A>";
...
switch($_GET[act]){
  default:		    
		...
		$negaranya = 'Semua negara';
			...
			echo "<h2>Approval EPO Sibling Kasubdit <br>$negaranya</h2>
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='epoKSib'>
...
<a href=?module=epoKSib>ALL</a> | <a href=?module=epoKSib&status=1 style=\"color : #B1BF19\">WAITING</a> | <a href=?module=epoKSib&status=2 style=\"color : green\">APPROVED</a> | 
	<a href=?module=epoKSib&status=0 style=\"color : #800000\">REJECTED</a> <BR>

		<form method=POST enctype='multipart/form-data' action='./aksi_id_cardK_sib.php?module=epoKSib&act=update'>
          <table width=100%>
  ...
  $p      = new Paging;
	...
	if (isset($_GET[namadiplomat])){
		...
		$tampil=mysql_query("select a.*, c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_EPO,'%d.%m.%Y') as TGL_AWAL_EPO,  date_format(a.TGL_AKHIR_EPO,'%d.%m.%Y') as TGL_AKHIR_EPO from v_epo_sib a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING   where c.NM_DIPLOMAT like '%".$_GET[namadiplomat]."%' and c.NEGARA like '%$neg%' and  a.ST_EPO_K =  ".$_GET[status]." ORDER BY a.ID_EPO_S desc limit $posisi,$batas");

	}
	else
    {

	$tampil=mysql_query("select a.*, c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_EPO,'%d.%m.%Y') as TGL_AWAL_EPO,  date_format(a.TGL_AKHIR_EPO,'%d.%m.%Y') as TGL_AKHIR_EPO from v_epo_sib a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING   where c.NEGARA like '%$neg%' and  a.ST_EPO_K =  ".$_GET[status]." ORDER BY a.ID_EPO_S desc limit $posisi,$batas");
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				echo "<tr>
						<td></td>
						<td></td>
						<td><input type='button' name='updateStatus' onClick=updateEpo($r[ID_EPO_S],$noP,'K','5'); value='Submit'></td>
					  </tr>";
...
*/				echo "  
	...
	if (isset($_GET[status])){
		if (isset($_GET[namadiplomat]))
		{
			...
			$jmldata =mysql_num_rows(mysql_query("select a.*, c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_EPO,'%d.%m.%Y') as TGL_AWAL_EPO,  date_format(a.TGL_AKHIR_EPO,'%d.%m.%Y') as TGL_AKHIR_EPO from v_epo_sib a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING   where c.NM_DIPLOMAT like '%".$_GET[namadiplomat]."%' and c.NEGARA like '%$neg%' and  a.ST_EPO_K =  ".$_GET[status]));

		
		}else{

			$jmldata =mysql_num_rows(mysql_query("select a.*, c.PEKERJAAN,c.NM_KNT_PERWAKILAN, c.NM_JNS_RELASI,date_format(a.TGL_AWAL_EPO,'%d.%m.%Y') as TGL_AWAL_EPO,  date_format(a.TGL_AKHIR_EPO,'%d.%m.%Y') as TGL_AKHIR_EPO from v_epo_sib a LEFT join v_sibling c on a.ID_SIBLING=c.ID_SIBLING   where c.NEGARA like '%$neg%' and  a.ST_EPO_K =  ".$_GET[status]));
	...
	if (isset($_GET[status])){
	$ilink = "?module=epoKSib&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]&status=$_GET[status]"; }
	else
	{$ilink = "?module=epoKSib&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]"; }
  ...
  case "cari":
    ...
    while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				&nbsp <a href=?module=epoKSib&negara=$r[NEGARA]>$r[NEGARA] </a></td><td>";
