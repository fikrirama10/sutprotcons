...
switch($_GET[act]){
  ...
  $sql= "SELECT DISTINCT `m_negara`.`ID_NEGARA`, `m_negara`.`NEGARA`, `m_kantor_perwakilan`.* FROM (`m_kantor_perwakilan`)
        LEFT JOIN `m_negara` ON `m_kantor_perwakilan`.`ID_NEGARA` = `m_negara`.`ID_NEGARA` WHERE (`m_kantor_perwakilan`.`ID_JNS_PERWAKILAN` = '2' OR `m_kantor_perwakilan`.`ID_JNS_PERWAKILAN` = '6') AND `m_negara`.`ID_NEGARA` IS NOT NULL AND `m_kantor_perwakilan`.`ID_NEGARA` !=('79' OR '1') AND `m_kantor_perwakilan`.`KODE_AGENDA`!='OI'  AND `m_kantor_perwakilan`.`NM_KNT_PERWAKILAN` NOT LIKE '%economic%'
...
//Subquery return more than 1 row: 1 ID_DIPLOMAT have more than 1 ID_SIBLING
    // $sql = "SELECT `diplomat`.*, `permit_diplomat`.`TGL_AKHIR_PERMIT`,(select MAX(TGL_AKHIR_PERMIT) from `permit_diplomat` where `permit_diplomat`.`ID_DIPLOMAT` = `diplomat`.`ID_DIPLOMAT` and kd_workflow>=3 limit 1) as TGL_AKHIR_CARD, `rank`.`OFFICIAL_NM`,
    // (select distinct NM_SIBLING from sibling where sibling.ID_DIPLOMAT=diplomat.ID_DIPLOMAT and ID_JNS_RELASI='3') as NM_SPOUSE,
    // (select distinct JK from sibling where sibling.ID_DIPLOMAT=diplomat.ID_DIPLOMAT and ID_JNS_RELASI='3') as JK_SPOUSE
    //   FROM (`diplomat`)
    ...
    // $sqlx="SELECT `diplomat`.*, `permit_diplomat`.`TGL_AKHIR_PERMIT`,(select MAX(TGL_AKHIR_PERMIT) from `permit_diplomat` where `permit_diplomat`.`ID_DIPLOMAT` = `diplomat`.`ID_DIPLOMAT` and kd_workflow>=3 limit 1) as TGL_AKHIR_CARD, `rank`.`OFFICIAL_NM`, `permit_diplomat`.`ST_AKTIF`,
    // (SELECT sibling.NM_SIBLING from sibling
    //     LEFT JOIN permit_sibling on sibling.ID_SIBLING=permit_sibling.ID_SIBLING
    //     WHERE permit_sibling.TGL_AKHIR_PERMIT>=CURDATE()
    //         AND permit_sibling.ST_AKTIF is null
    //         AND sibling.ID_JNS_RELASI=3
    //         AND sibling.ID_DIPLOMAT=diplomat.ID_DIPLOMAT) as NM_SPOUSE,
    //  (SELECT sibling.JK from sibling
    //     LEFT JOIN permit_sibling on sibling.ID_SIBLING=permit_sibling.ID_SIBLING
    //     WHERE permit_sibling.TGL_AKHIR_PERMIT>=CURDATE()
    //         AND permit_sibling.ST_AKTIF is null
    //         AND sibling.ID_JNS_RELASI=3
    //         AND sibling.ID_DIPLOMAT=diplomat.ID_DIPLOMAT) as JK_SPOUSE
    //   FROM (`diplomat`)
    //   JOIN `rank` ON `diplomat`.`ID_RANK`=`rank`.`ID_RANK`
    //   JOIN `permit_diplomat` ON `permit_diplomat`.`ID_DIPLOMAT`=`diplomat`.`ID_DIPLOMAT`
    //   WHERE `diplomat`.`ID_KNT_PERWAKILAN` = '$idt' AND `MENGGANTIKAN_SIAPA` IS NOT NULL
    ...
    //buat cek if id_jns_pwk =2/6
    // $sqlx="SELECT * FROM v_dipsib_list
    //           WHERE ID_CARD !='' AND ID_CARD is not null
    //           AND ID_KNT_PERWAKILAN='$idt'
    //       ORDER BY ID_RANK ASC";

    //Union dgn tabel nonresidence
    $sqlx="SELECT ID_DIPLOMAT, ID_RANK, NEGARA, NM_STATES, TGL_CREDENTIAL, TGL_TIBA, OFFICIAL_NM, RANK, ID_KNT_PERWAKILAN, NM_DIPLOMAT, KANTOR, NM_KNT_PERWAKILAN, ALAMAT, TELP, FAX, EMAIL, WEB, OFFHOURS, NATIONALDAY, KET_NATIONALDAY, OFFICIAL_PEKERJAAN, NM_SIBLING, JK_DIPLOMAT, JK_SIBLING, KET_DEAN, ID_NEGARA, ID_JNS_PERWAKILAN, KATEGORI_PEMOHON
      FROM v_dipsib_list
      WHERE ID_CARD !='' AND ID_CARD is not null
      AND v_dipsib_list.ID_KNT_PERWAKILAN='$idt'
    UNION SELECT  a.ID_DIPLOMAT, a.ID_RANK, c.NEGARA, c.NM_STATES, date_format(a.TGL_CREDENTIAL,'%M %D, %Y'), a.TGL_TIBA, d.OFFICIAL_NM, d.RANK, a.ID_KNT_PERWAKILAN, a.NM_DIPLOMAT, b.OFFICIAL_NM AS KANTOR, b.NM_KNT_PERWAKILAN, b.ALAMAT, b.TELP, b.FAX, b.EMAIL, b.WEB, b.OFFHOURS, date_format(b.NATIONALDAY,'%M %D') AS NATIONALDAY, b.KET_NATIONALDAY, a.OFFICIAL_PEKERJAAN, e.NM_SIBLING, a.JK AS JK_DIPLOMAT, e.JK AS JK_SIBLING, a.KET_DEAN, a.ID_NEGARA, b.ID_JNS_PERWAKILAN, a.KATEGORI_PEMOHON
    FROM nonresidence a
      ...
      LEFT JOIN nonresidence_sibling e ON a.ID_DIPLOMAT = e. ID_DIPLOMAT
      WHERE a.ID_KNT_PERWAKILAN='$idt'
    ORDER BY RANK ASC,TGL_TIBA ASC";

    $edit = mysql_query($sqlx);


    $dubes= "SELECT COUNT(ID_RANK) FROM ($sqlx) AS X WHERE ID_RANK='1'";
...
//Query untuk data perwkilan
    ...
    //Jika ada Keppri, munculkan tanggal credential
     $neg = "SELECT * FROM ($sqlx) AS Y
       WHERE ID_RANK='1' AND ID_KNT_PERWAKILAN = '$idt' ";
    }else{
    //Tidak ada Keppri, tidak ada tanggal credential
     $neg = "SELECT * FROM ($sqlx) AS Z
      WHERE ID_KNT_PERWAKILAN = '$idt' ";
    }
   ...
   $rneg = mysql_fetch_array($dneg);
	// while($rneg = mysql_fetch_array($dneg))
	// {
     $negara = $rneg['ID_NEGARA'];
     if ($adubes['0']>0){
      $aDate0 = date("<\b>F jS, Y</\b>", strtotime($rneg['TGL_CREDENTIAL']));
  ...
  //   if (isset($rneg['NATIONALDAY'])){
    ...
    echo "
        ...
        <form method=POST action='./aksi_negara.php?module=perutusan&act=input' enctype='multipart/form-data'>
          ...
          <tr><td width=120>National Day</td >  <td colspan=2> :   $rneg[KET_NATIONALDAY], $rneg[NATIONALDAY]</td></tr>
          </table>
          <table width=90%>
          <tr>
            <th>NO.</th>
  ...
  //}else{
    ...
    //       <table width=90%>
    //       <tr><td colspan=3 align='center'>
    //           <b>$result[NEGARA]</b> <br>
    //           $result[NM_KNT_PERWAKILAN] <br>
    //       </td ></tr>
    //       <tr><td rowspan=4>Chancery</td >
    //           <td width=50 colspan=2>$result[ALAMAT]</td></tr>
    //           <td width=50>Phone</td><td >: $result[TELP] </td></tr>
    //           <td>Fax</td><td >: $result[FAX]</td></tr>
    //           <td>Email</td><td >: $result[EMAIL]</td></tr>
    //       <tr><td >Office Hours</td>     <td colspan=2> : $result[OFFHOURS]</td></tr>
    //       <tr><td width=120>National Day</td >  <td colspan=2> :   $result[KET_NATIONALDAY], $result[NATIONALDAY]</td></tr>
    //       </table>
    //       <table width=90%>
    //       <tr>
    //         <th>NO.</th>
  ...
  //}
	...
	{
            $no++;
            if ($data['TGL_TIBA'] == null OR $data['TGL_TIBA']==''){
              $aDate = '-';
            }else{
						...
						//untuk HS
						$tHS = "";
						if($data['JK_DIPLOMAT']=='p'){
							$tHS = 'Mrs.';
						}elseif($data['JK_DIPLOMAT']=='l'){
							$tHS = 'Mr.';
						}
		  ...
		  <tr>
      <td width=5%>$no</td>
      <td width=30%>"; if ($data['KATEGORI_PEMOHON'] != NULL){
        //if ($data['ID_JNS_PERWAKILAN'] == 2){
            echo "<a href=?module=diplomat&act=viewdiplomat&idt=$data[ID_DIPLOMAT]&negara=$_GET[negara]> $tHS $data[NM_DIPLOMAT] </a>";
        }elseif ($data['KATEGORI_PEMOHON'] == NULL){
        //}elseif ($data['ID_JNS_PERWAKILAN'] == 6){
            echo "<a href=?module=nonresidence&act=viewnonresidence&idt=$data[ID_DIPLOMAT]&negara=$_GET[negara]> $tHS $data[NM_DIPLOMAT] </a>";
        }
        echo "<br> $tSP $nSP</td>
					<td width=30%>$data[OFFICIAL_NM] <br />$data[OFFICIAL_PEKERJAAN]</td>
					<td width=20%> $aDate</td>
					<td width=15% align=center>"; if ($data['KATEGORI_PEMOHON'] != NULL){
                echo "<a href=?module=diplomat&act=editdiplomat&idt=$data[ID_DIPLOMAT]&idb=$_GET[idt]&negara=$_GET[negara]>Edit</a> </td></tr>";
            }elseif ($data['KATEGORI_PEMOHON'] == NULL){
                echo "<a href=?module=nonresidence&act=editnonresidence&idt=$data[ID_DIPLOMAT]&idb=$_GET[idt]&negara=$_GET[negara]>Edit</a></td></tr>";
            }
	}
	echo "</table>
  </form>
...
//Query check kalau ada Konjen
  ...
  if ($c['0']!=0){
      while($consul = mysql_fetch_array($consuls)){
        ...
        // }else{
          $pconsuls=mysql_query("(SELECT * FROM v_dipsib_list WHERE ID_CARD !=''
                                  AND ID_CARD IS NOT NULL
                                  AND ID_KNT_PERWAKILAN='$kantor')
                              UNION ALL
                                (SELECT * FROM v_dipsib_list WHERE ID_KNT_PERWAKILAN = '$kantor')
                              ORDER BY ID_RANK ASC");
          ...
          <tr>
            ...
            $no = 0;
          	while($pconsul = mysql_fetch_array($pconsuls))
          	{
          						...
          						//untuk HS
          						$tHS = "";
          						if($pconsul['JK_DIPLOMAT']=='p'){
          							$tHS = 'Mrs.';
          						}elseif($pconsul['JK_DIPLOMAT']=='l'){
          							$tHS = 'Mr.';
          						}

          						//untuk spouse
          						$tSP  = "";
          						$nSP  = $pconsul['NM_SIBLING'];
          						$jkSP = $pconsul['JK_SIBLING'];
                    ...
                    <td width=30%><a href=?module=diplomat&act=viewdiplomat&idt=$pconsul[ID_DIPLOMAT]&negara=$_GET[negara]> $tHS $pconsul[NM_DIPLOMAT] </a>
                        <br> $tSP $nSP</td>
          					<td width=30%>$pconsul[OFFICIAL_NM] <br /> $pconsul[OFFICIAL_PEKERJAAN]</td>
          					<td width=20%> $aDate</td>
