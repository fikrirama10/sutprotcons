   ...
   // if (!empty($neg)){
	...
	if (isset($_GET['namapemohon']) || isset($_GET['indeksvisa']) || isset($_GET['pwk']) || isset($_GET['stat'])){
			...
			$sql="SELECT
										...
										FROM
											tbl_trans_otvis AS otvis $term_wh $np $iv $pwk $st order by status_permohonan desc, created_date desc limit $posisi,$batas";
										...
										*,
										(	SELECT
											...
											WHERE
												id_otvis = otvis.id_otvis) AS jml
									FROM
										tbl_trans_otvis AS otvis $stmhn_baru $id_pwk order by no_konsep_pwk desc limit $posisi,$batas";
		}
		else
		{
								...
								*,
								(	SELECT
									...
									WHERE
										id_otvis = otvis.id_otvis) AS jml
							FROM
								tbl_trans_otvis AS otvis $stmhn_baru $id_pwk order by status_permohonan desc, created_date desc limit $posisi,$batas";
		}
		//print_r($sql);
		$tampilbeda=mysql_query($sql);
