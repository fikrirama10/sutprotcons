<?php
	   echo "<a href=?module=rantorKD&act=cari&huruf=A>A</A> |	<a href=?module=rantorKD&act=cari&huruf=B>B</A> |	<a href=?module=rantorKD&act=cari&huruf=C>C</A> |	<a href=?module=rantorKD&act=cari&huruf=D>D</A> |	<a href=?module=rantorKD&act=cari&huruf=E>E</A> |	<a href=?module=rantorKD&act=cari&huruf=F>F</A> |	<a href=?module=rantorKD&act=cari&huruf=G>G</A> |	<a href=?module=rantorKD&act=cari&huruf=H>H</A> |	<a href=?module=rantorKD&act=cari&huruf=I>I</A> |	<a href=?module=rantorKD&act=cari&huruf=J>J</A> |	<a href=?module=rantorKD&act=cari&huruf=K>K</A> |	<a href=?module=rantorKD&act=cari&huruf=L>L</A> |	<a href=?module=rantorKD&act=cari&huruf=M>M</A> |	<a href=?module=rantorKD&act=cari&huruf=N>N</A> |	<a href=?module=rantorKD&act=cari&huruf=O>O</A> |	<a href=?module=rantorKD&act=cari&huruf=P>P</A> |	<a href=?module=rantorKD&act=cari&huruf=Q>Q</A> |	<a href=?module=rantorKD&act=cari&huruf=R>R</A> |	<a href=?module=rantorKD&act=cari&huruf=S>S</A> |	<a href=?module=rantorKD&act=cari&huruf=T>T</A> |	<a href=?module=rantorKD&act=cari&huruf=U>U</A> |	<a href=?module=rantorKD&act=cari&huruf=V>V</A> |	<a href=?module=rantorKD&act=cari&huruf=W>W</A> |	<a href=?module=rantorKD&act=cari&huruf=X>X</A> |	<a href=?module=rantorKD&act=cari&huruf=Y>Y</A> |	<a href=?module=rantorKD&act=cari&huruf=Z>Z</A>";
...
switch($_GET[act]){
  default:		    
		...
		echo "<h2>Approval Rantor Kantor Perwakilan Direktur <br>$negaranya</h2>
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='rantorKD'>
...
<a href=?module=rantorKD>ALL</a> | <a href=?module=rantorKD&status=1 style=\"color : #B1BF19\">WAITING</a> | <a href=?module=rantorKD&status=2 style=\"color : green\">APPROVED</a> | 
	<a href=?module=rantorKD&status=0 style=\"color : #800000\">REJECTED</a> <BR>

		<form method=POST enctype='multipart/form-data' action='./aksi_rantorKD.php?module=rantorKD&act=update'>
          <table width=100%>
 ...
 where  (negara like '".$neg."%' and NM_KNT_PERWAKILAN like '%".$_GET[namakantor]."%') and  ST_PERSETUJUAN =  ".$_GET[status]." ORDER BY TANGGAL_IZIN,NM_KNT_PERWAKILAN desc limit $posisi,$batas ");	
 ...
 where  (negara like '".$neg."%' ) and  ST_PERSETUJUAN =  ".$_GET[status]." ORDER BY TANGGAL_IZIN,ID_KNT_PERWAKILAN , NM_DIPLOMAT, ID_RANK desc limit $posisi,$batas ");	
	...
	$tampil=mysql_query("select ID_PENGGUNAA_FAS,ID_KNT_PERWAKILAN,NEGARA,NM_KNT_PERWAKILAN,ALAMAT,KOTA,NM_JNS_PERWAKILAN,NM_SUB_JNS,NO_POLISI,  ST_ASAL,TANGGAL_IZIN,NO_IZIN , ST_KENDARAAN,ST_PERSETUJUAN,ST_PERSETUJUAN_K,BENDERA from v_approval_rantork
 where  (negara like '".$neg."%') and  ST_PERSETUJUAN =  ".$_GET[status]." ORDER BY TANGGAL_IZIN,NM_KNT_PERWAKILAN desc limit $posisi,$batas ");
 ...
 where  (negara like '".$neg."%' )  ORDER BY TANGGAL_IZIN,NM_KNT_PERWAKILAN desc limit $posisi,$batas ");
	...
	if (isset($_GET[status])){
		if (isset($_GET[namakantor]))
		{
			$jmldata =mysql_num_rows(mysql_query("select ID_PENGGUNAA_FAS from v_approval_rantork where  (negara like '".$neg."%' and NM_KNT_PERWAKILAN like '%".$_GET[namakantor]."%') and  ST_PERSETUJUAN =  ".$_GET[status]));

 

		}else{

			$jmldata =mysql_num_rows(mysql_query("select ID_PENGGUNAA_FAS from v_approval_rantork where  (negara like '".$neg."%' ) and  ST_PERSETUJUAN =  ".$_GET[status]));
		}
	}else{
		if (isset($_GET[namakantor]))
		{
	...
	if (isset($_GET[status])){
	$ilink = "?module=rantorKD&negara=$_GET[negara]&namakantor=$_GET[namakantor]&status=$_GET[status]"; }
	else
	{$ilink = "?module=rantorKD&negara=$_GET[negara]&namakantor=$_GET[namakantor]"; }
  ...
  case "cari":
	...
	echo "<h2>Rantor Kantor - Pilih Negara</h2> 
	<table width=100%>
    ...
    while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				&nbsp <a href=?module=rantorKD&negara=$r[NEGARA]>$r[NEGARA] </a></td><td>";
