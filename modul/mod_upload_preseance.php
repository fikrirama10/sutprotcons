...
switch($_GET[act]){
  ...
  default:
		...
		echo "<h2 align='center'>ORDER OF PRESEANCE <br> THE HEAD OF DIPLOMATIC MISSIONS</h2>
          <form method=POST action='./aksi_preseance.php?module=upload_preseance&act=input' enctype='multipart/form-data'>
            <table border=0>
              <tr><td>Versi dokumen per tanggal</td>
              <td colspan=\"2\">
                    <DIV id=\"tgl\"> <script>DateInput('tanggal', true, 'YYYY-MM-DD')</script></div></td>
              </tr>
              <tr><td>Unggah data <i>Order of Preseance</i> &nbsp;</td>
                <td> : <input type=file size=40 name=upload_preseance></td>
              </tr>
              <tr><td colspan=3 align=right><input type=submit value=Unggah></td></tr>
            </table>
          </form>
          ...
          <table width=100% id=cariStayPermit name=cariStayPermit class=display>
              <thead><tr><th width=5%>no</th>
        		      ...
        		      <th width=35%>Waktu Unggah</th>
        		      <th width=20%>Status</th>
                  <th width=20%>Aksi</th></tr></thead><tbody>";


    $tampil0="SELECT * FROM tbl_data_preseance ORDER BY IS_AKTIF DESC";
    $tampil=mysql_query($tampil0);

    $no = $posisi+1;
    while($r=mysql_fetch_array($tampil)){
      $oDate = new DateTime($r['TGL_UPLOAD']);
      ...
      echo "<tr><td>$no</td>
              <td><a href='../diplist/$r[NAMA_FILE]' target='_blank'>$r[NAMA]</a></td>
				      <td align=center>$sDate</td>
              <td align=center>$status</td>
              <td align=center><a href='./aksi_preseance.php?module=upload_preseance&act=hapus&idt=$r[ID]' enctype='multipart/form-data' onClick=\"return confirm('Apakah Anda benar-benar akan menghapus $r[NAMA]?')\">Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</tbody></table>";
