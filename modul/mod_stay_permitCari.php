...
// 	if (empty($_GET[namadiplomat]) and empty($_GET[nopermit]) and $_GET[ID_NEGARA] = "1" and $_GET[kantordiplomat] != "1" and $_GET[ID_JNS_PERMIT] = "0" and ($_GET[thn1] = "1" or $_GET[thn2] = "1")) {
	...
	$tampil=mysql_query("select a.id_permit,b.ID_DIPLOMAT,b.NM_DIPLOMAT,b.NM_KNT_PERWAKILAN,b.PEKERJAAN, a.NO_IZIN_PERMIT, date_format(a.TGL_AWAL_PERMIT,'%d.%m.%Y') as TGL_AWAL_PERMIT ,date_format(a.TGL_AKHIR_PERMIT,'%d.%m.%Y') as TGL_AKHIR_PERMIT,
	a.KD_JNS_PERMIT, b.KEWARGANEGARAAN, b.ALAMATIN, year(a.TGL_AKHIR_PERMIT) as thn 
from v_diplomat b LEFT join v_stay_permit a on b.id_diplomat = a.id_diplomat 
LEFT JOIN v_max_tgl_akhir_permit c on a.ID_DIPLOMAT = c.ID_DIPLOMAT and a.TGL_AKHIR_PERMIT = c.TGL_AKHIR_PERMIT and a.TGL_AWAL_PERMIT = c.TGL_AWAL_PERMIT
where b.ID_DIPLOMAT > 1 and b.NM_KNT_PERWAKILAN != '' and a.NO_IZIN_PERMIT != '' and b.NEGARA != '' and a.TGL_AKHIR_PERMIT >= DATE(NOW()) 
	group by a.id_permit,b.ID_DIPLOMAT,b.NM_DIPLOMAT,a.NO_IZIN_PERMIT, a.TGL_AWAL_PERMIT, a.TGL_AKHIR_PERMIT order by a.id_permit desc");
	
		
	/* echo "select a.id_permit,b.ID_DIPLOMAT,b.NM_DIPLOMAT,b.NM_KNT_PERWAKILAN,b.PEKERJAAN, a.NO_IZIN_PERMIT, date_format(a.TGL_AWAL_PERMIT,'%d.%m.%Y') as TGL_AWAL_PERMIT ,date_format(a.TGL_AKHIR_PERMIT,'%d.%m.%Y') as TGL_AKHIR_PERMIT,
	a.KD_JNS_PERMIT, b.NEGARA, b.ALAMATIN from v_stay_permit a right join v_diplomat b on a.id_diplomat=b.id_diplomat where b.ID_DIPLOMAT > 1 and b.NM_KNT_PERWAKILAN != '' and b.NEGARA != '' and year(a.TGL_AKHIR_PERMIT) >= $yr 
	group by a.id_permit,b.ID_DIPLOMAT,b.NM_DIPLOMAT,a.NO_IZIN_PERMIT, a.TGL_AWAL_PERMIT, a.TGL_AKHIR_PERMIT order by year(a.TGL_AKHIR_PERMIT) asc";*/
	//echo time();
	echo "<table style=width:100% name=cariStayPermit id=cariStayPermit class=display	>
					<thead>
					<tr>
						<!--<th width=30>NO</th>-->
					  <th>KANTOR</th>					  
					...
					<tbody>
          <!--<tr><th width=30>no</th><th width=80>NO PERMIT</th><th width=40>JNS PERMIT</th><th>NAMA LENGKAP</th><th width=120>NEGARA</th><th width=120>KANTOR PERWAKILAN</th><th width=100>JABATAN</th><th width=80>TGL BERLAKU</th><th width=70>AKSI</th></tr>-->";
    $no = $posisi+1;
    while($r=mysql_fetch_array($tampil)){
			$today = time();
			$hari= date('d.m.y');
      ...
      echo "
              ...
              <td>$r[PEKERJAAN]</td>
							...
							<!--<td>$status</td>-->";						
							//if($level != '99'){
					//echo "<td><a href=?module=diplomat&act=editdiplomat&idt=$r[ID_DIPLOMAT]&negara=$_GET[negara]>Edit</a></td>";
				//}			
		        echo "</tr>";
      $no++;
    }
...
//
	 ...
	 //
   // $ilink = "?module=staypermitc&negara=$_GET[negara]&kantordiplomat=$_GET[kantordiplomat]";
