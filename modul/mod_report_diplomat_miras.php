  ...
  default:
		...
		echo "<link rel='stylesheet' href='../config/jquery-ui-1.12.1.custom/themes/base/jquery-ui.css'"
			...
			<form method=get action='./deplu.php?' enctype='multipart/form-data'>
				 <input type=hidden name=module value='report_staf'>
...
. "<script>
 ...
 if(document.getElementById('negara').value == '' && document.getElementById('perwakilan').value == '' && document.getElementById('tgl_cr_aju1').value == '' && document.getElementById('tgl_cr_aju2').value == '' && document.getElementById('nm_diplomat').value == '' && document.getElementById('setuju').checked==false && document.getElementById('tolak').checked==false){
  ...
  window.location='report.php?go=miras_staf&negara='+a+'&perwakilan='+b+'&tgl_cr_aju1='+c+'&tgl_cr_aju2='+d+'&nm_diplomat='+e+'&setuju='+f+'&tolak='+g;
}
}
 </script>";
 //break;      
...
//case "cari";
 ...
 echo"<table width=100%>
          <tr><th width=2%>no</th><th width=15%>Nama</th><th width=21%>KANTOR</th><th width=21%>NEGARA</th><th width=5%>TGL PENGAJUAN</th><th width=10%>NOTA PENGAJUAN</th><th width=10%>NOTA JAWABAN</th><th width=22%>PENGAJUAN PRODUK</th><th width=5%>STATUS</th></tr>";
    ...
    echo"Hasil pencarian berdasarkan : $tgl_pengajuan_rpt $negara_rpt $knt_perwakilan $nm_diplomat_rpt $setuju_tolak";
		$sql="SELECT id_pengajuan_miras,kd_pengajuan_miras,NM_DIPLOMAT, NM_KNT_PERWAKILAN, NEGARA, tgl_nota_pengajuan,no_nota_pengajuan, no_nota_jawaban, CONCAT_WS('','Spirit : ', jumlah_pengajuan_spirit, ', ', 'Anggur : ', jumlah_pengajuan_anggur, ', ', 'Rokok : ',jumlah_pengajuan_rokok) as pengajuan_produk, status_pengajuan, nama_file_aju,tipe_file_aju, lokasi_file_pengajuan,nama_file_jawab,tipe_file_jwb, lokasi_file_jawaban, keterangan, triwulan, tahun, tgl_nota_jawaban FROM v_report_miras_staf where status_pengajuan is not NULL $w1 $w2 $w3 $w4 $w5 order by tgl_nota_pengajuan desc limit $posisi,$batas";
...
//echo $sql;
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      $nm= json_encode($r[NM_DIPLOMAT]);
      $file_tipe_aju=$r[nama_file_aju].'.'.$r[tipe_file_aju];
      $file_tipe_jwb=$r[nama_file_jawab].'.'.$r[tipe_file_jwb];
      
      $produk=mysql_query("select id_produk, jumlah from tbl_detil_pengajuan_miras_staf where id_pengajuan_miras = $r[id_pengajuan_miras]");
       while($prdk=mysql_fetch_array($produk)){
        ...
        $file_staf_aju=mysql_query("select lokasi_file, nama_file, jenis, tipe_file from tbl_file_miras_staf where id_pengajuan_miras = $r[id_pengajuan_miras] and jenis='file_pengajuan'");
...
$file_staf_jwb=mysql_query("select lokasi_file, nama_file, jenis, tipe_file from tbl_file_miras_staf where id_pengajuan_miras = $r[id_pengajuan_miras] and jenis='file_jawaban'");
      ...
      echo "<tr><td>$no</td>                                     
                <td><a href='#' id='mylink' onClick='loadPopupBoxMiras(2,$kd1,$r[id_pengajuan_miras],$pengajuan,$jwb,$ket,$sts,$tw,$th,$tgl_aju,$tgl_jb,$jml_spirit,$jml_anggur,$jml_rkk,$lokasi_aju,$nama_aju,$tipe_fl_aju,$lokasi_jwb,$nama_jwb,$tipe_fl_jwb,$nm);'>$r[NM_DIPLOMAT]</a></td>
    ...
    echo "</table>";
		...
		$jmldata =mysql_num_rows(mysql_query("SELECT NM_DIPLOMAT, NM_KNT_PERWAKILAN, NEGARA, tgl_nota_pengajuan,no_nota_pengajuan, no_nota_jawaban, CONCAT_WS('','Spirit : ', jumlah_pengajuan_spirit, ', ', 'Anggur : ', jumlah_pengajuan_anggur, ', ', 'Rokok : ',jumlah_pengajuan_rokok) as pengajuan_produk, status_pengajuan FROM v_report_miras_staf where status_pengajuan is not NULL $w1 $w2 $w3 $w4 order by tgl_nota_pengajuan DESC"));
	
	$jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);    
   $ilink = "?module=report_staf&nm_diplomat=$_GET[nm_diplomat]&tgl_cr_aju1=$_GET[tgl_cr_aju1]&tgl_cr_aju2=$_GET[tgl_cr_aju2]&perwakilan=$_GET[perwakilan]&negara=$_GET[negara]&setuju=$_GET[setuju]&tolak=$_GET[tolak]"; 
