...
class epo {
	...
	var $idimg;
	var $representative;
	public function epo($iddiplomat, $jns) {
			//$sql = "select c.NM_DIPLOMAT,a.NO_EPO,a.ID_EPO,date_format(a.TGL_AKHIR_EPO,'%d') as tgl,date_format(a.TGL_AKHIR_EPO,'%c') as bln,date_format(a.TGL_AKHIR_EPO,'%Y') as thn, b.JNS_PASPOR, b.KD_JNS_PASPOR from v_epo a,diplomat c, m_jns_paspor b where a.ID_DIPLOMAT=c.ID_DIPLOMAT and b.ID_JNS_PASPOR = c.ID_JNS_PASPOR and a.ID_DIPLOMAT= $iddiplomat and a.ID_EPO = (select max(d.ID_EPO) from epo_diplomat d where d.ID_DIPLOMAT= $iddiplomat AND d.KD_WORKFLOW>=1)";

		if($jns != 'manual'){
		$sql = "select c.NM_DIPLOMAT,a.NO_EPO,a.ID_EPO,date_format(a.TGL_AKHIR_EPO,'%d') as tgl,date_format(a.TGL_AKHIR_EPO,'%c') as bln,date_format(a.TGL_AKHIR_EPO,'%Y') as thn, SUBSTR(a.NO_EPO,11,1) as KD_JNS_PASPOR from v_epo a,diplomat c where a.ID_DIPLOMAT=c.ID_DIPLOMAT and a.ID_DIPLOMAT= $iddiplomat and a.ID_EPO = (select max(d.ID_EPO) from epo_diplomat d where d.ID_DIPLOMAT= $iddiplomat AND d.KD_WORKFLOW>=1)";
      }
      elseif($jns == 'manual'){
		$sql = "select c.nama as NM_DIPLOMAT,a.NO_EPO,a.ID_EPO,date_format(a.TGL_AKHIR_EPO,'%d') as tgl,date_format(a.TGL_AKHIR_EPO,'%c') as bln,date_format(a.TGL_AKHIR_EPO,'%Y') as thn, SUBSTR(a.NO_EPO,11,1) as KD_JNS_PASPOR from v_epo a,tbl_trans_otvis c where a.ID_OTVIS=c.id and a.ID_OTVIS= $iddiplomat and a.ID_EPO = (select max(d.ID_EPO) from epo_diplomat d where d.ID_OTVIS= $iddiplomat AND d.KD_WORKFLOW>=1)";
      }
		...
		$nomor_epo  		= $data['NO_EPO'];
		$nama_ok			= htmlspecialchars_decode($data['NM_DIPLOMAT'], ENT_QUOTES);
		//$panjangNama =14;
		...
		$this->formattingprint ( $_POST ['TGL_CETAK'],$data['NO_EPO'],$nama_ok,$data['tgl'],$data['bln'],$data['thn'], $data['KD_JNS_PASPOR']);
	}
