<?php
	   echo "<br><b><a href=?module=rantorKimpor&act=cari&huruf=A>A</A> |	<a href=?module=rantorKimpor&act=cari&huruf=B>B</A> |	<a href=?module=rantorKimpor&act=cari&huruf=C>C</A> |	<a href=?module=rantorKimpor&act=cari&huruf=D>D</A> |	<a href=?module=rantorKimpor&act=cari&huruf=E>E</A> |	<a href=?module=rantorKimpor&act=cari&huruf=F>F</A> |	<a href=?module=rantorKimpor&act=cari&huruf=G>G</A> |	<a href=?module=rantorKimpor&act=cari&huruf=H>H</A> |	<a href=?module=rantorKimpor&act=cari&huruf=I>I</A> |	<a href=?module=rantorKimpor&act=cari&huruf=J>J</A> |	<a href=?module=rantorKimpor&act=cari&huruf=K>K</A> |	<a href=?module=rantorKimpor&act=cari&huruf=L>L</A> |	<a href=?module=rantorKimpor&act=cari&huruf=M>M</A> |	<a href=?module=rantorKimpor&act=cari&huruf=N>N</A> |	<a href=?module=rantorKimpor&act=cari&huruf=O>O</A> |	<a href=?module=rantorKimpor&act=cari&huruf=P>P</A> |	<a href=?module=rantorKimpor&act=cari&huruf=Q>Q</A> |	<a href=?module=rantorKimpor&act=cari&huruf=R>R</A> |	<a href=?module=rantorKimpor&act=cari&huruf=S>S</A> |	<a href=?module=rantorKimpor&act=cari&huruf=T>T</A> |	<a href=?module=rantorKimpor&act=cari&huruf=U>U</A> |	<a href=?module=rantorKimpor&act=cari&huruf=V>V</A> |	<a href=?module=rantorKimpor&act=cari&huruf=W>W</A> |	<a href=?module=rantorKimpor&act=cari&huruf=X>X</A> |	<a href=?module=rantorKimpor&act=cari&huruf=Y>Y</A> |	<a href=?module=rantorKimpor&act=cari&huruf=Z>Z</A> </b>";
...
switch($_GET[act]){
  default:		    
		...
		echo "<h2>Import Rantor Kantor<br>Pilih Kantor Perwakilan - $negaranya</h2>
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td><a href=?module=rantorKimpor&act=lihat_rantorKimpor&idt=$r[ID_KNT_PERWAKILAN]&negara=$_GET[negara]> $r[NM_KNT_PERWAKILAN] </a></td>
                <td>$r[ALAMAT]</td>
				<td>$r[TELP]</td>		
				<td>$r[KOTA]</td>		
   ...
   $ilink = "?module=rantorKimpor&negara=$_GET[negara]"; 
    $linkHalaman = $p->navHalaman($ilink,$_GET[halaman], $jmlhalaman);

    echo "<div id=paging>$linkHalaman</div><br>";
    break;

  case "lihat_rantorKimpor":
	$idt = $_GET[idt];
	...
	echo " <input type=button value='Tambah' onclick=location.href='?module=rantorKimpor&act=tambah_rantorKimpor&idt=$idt&negara=$_GET[negara]'>
    ...
    $tampil=mysql_query("select ID_PENGGUNAA_FAS,ID_JNS_FASILITAS,ID_DIPLOMAT,ID_KNT_PERWAKILAN,DATE_FORMAT(TGL_IZIN_IMPOR,'%d %M %Y') AS TGL_IZIN_IMPOR,DESKRIPSI,NO_IZIN_IMPOR,QTY,ST_PERSETUJUAN,ST_PERSETUJUAN_K,NO_POLISI,TAHUN,MEREK,NO_MESIN from penggunaan_fasilitas  where ID_DIPLOMAT = 1 and ID_KNT_PERWAKILAN = $idt and (ID_JNS_FASILITAS=1) and (NO_IZIN_IMPOR is not null) order by ID_PENGGUNAA_FAS ");
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td>$no</td>
		...
		echo "</td>
				...
				<td><a href=?module=rantorKimpor&act=edit_rantorKimpor&idt=$r[ID_PENGGUNAA_FAS]&idd=$idt&negara=$_GET[negara]>Edit</a> | 
		            <a href=./aksi_rantorKimpor.php?module=rantorKimpor&act=hapus&idt=$r[ID_PENGGUNAA_FAS]&idd=$idt&negara=$_GET[negara] onClick=\"return confirm('Apakah Anda benar-benar akan menghapus Rantor $r[DESKRIPSI]?')\">Hapus</a></td>
				</tr>";
 ...
 case "tambah_rantorKimpor":
	$idt = $_GET[idt];
    $input = mysql_query("select ID_KNT_PERWAKILAN,NEGARA,NM_KNT_PERWAKILAN,ALAMAT,KOTA,TELP,BENDERA,NM_JNS_PERWAKILAN,FAX,KET FROM v_kantor_perwakilan  where ID_KNT_PERWAKILAN  = $idt  ");
	$r    = mysql_fetch_array($input);
	
	 echo "<h2 >Impor Rantor Kantor - Tambah</h2>";
	 echo "<table width=100%>
			<tr><td  width=160>Asal Negara</td>  <td > : $r[NEGARA] &nbsp <img src=\"../images/bendera/".$r[BENDERA]."\" class=\"thumbborder\" width=\"22\" height=\"15\" /></td></tr>
			<tr><td>Jenis Kantor</td>     <td> : $r[NM_JNS_PERWAKILAN]</td></tr> 
			<tr><td>Nama Kantor</td>     <td> : $r[NM_KNT_PERWAKILAN]</td></tr>
	...
	echo "<form method=POST enctype='multipart/form-data' action='./aksi_rantorKimpor.php?module=rantorKimpor&act=input&negara=$_GET[negara]'>
 ...
 case "edit_rantorKimpor":
  ...
  $input = mysql_query("select ID_KNT_PERWAKILAN,NEGARA,NM_KNT_PERWAKILAN,ALAMAT,KOTA,TELP,BENDERA,NM_JNS_PERWAKILAN,FAX,KET FROM v_kantor_perwakilan  where ID_KNT_PERWAKILAN  = $idd  ");
	$r    = mysql_fetch_array($input);
	 ...
	 echo "<h2 >Impor Rantor Kantor - Edit</h2>";
	 echo "<table width=100%>
			<tr><td  width=160>Asal Negara</td>  <td > : $r[NEGARA] &nbsp <img src=\"../images/bendera/".$r[BENDERA]."\" class=\"thumbborder\" width=\"22\" height=\"15\" /></td></tr>
			<tr><td>Jenis Kantor</td>     <td> : $r[NM_JNS_PERWAKILAN]</td></tr> 
			<tr><td>Nama Kantor</td>     <td> : $r[NM_KNT_PERWAKILAN]</td></tr>
...
NO_IZIN_JUAL,TGL_IZIN_JUAL,ALASAN_PENJUALAN,REKOMENDASI_BENGKEL,NAMA_PEMBELI,ALAMAT_PEMBELI,NO_KTP_PEMBELI from penggunaan_fasilitas where ID_PENGGUNAA_FAS = $idt ");   
	...
	echo "<form method=POST enctype='multipart/form-data' action='./aksi_rantorKimpor.php?module=rantorKimpor&act=update&idt=$idt&negara=$_GET[negara]'>
          <input type=hidden name=ID_DIPLOMAT value='1'>
     	  <input type=hidden name=ID_KNT_PERWAKILAN value='$r[ID_KNT_PERWAKILAN]'>
		  <table width=100%>		  		 
		<th colspan=2 align = left>Import Rantor</th>	  
 ...
 break;
  case "cari":
	...
	echo "<h2>Import Rantor Kantor - Pilih Negara</h2>
<table width=100%>
    ...
    while($r=mysql_fetch_array($tampil)){
		...
		echo "<tr><td>$no</td>
				...
				&nbsp <a href=?module=rantorKimpor&negara=$r[NEGARA]>$r[NEGARA] </a></td><td>";
