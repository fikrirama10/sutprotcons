...
switch($_GET[act]){
  ...
  default:
		...
		echo "<h2 align='center'>PERMANENT MISSIONS TO ASEAN</h2>
			 <br>
          <input type=button value='Tambah' onclick=location.href='?module=kantor&act=tambahkantor'><p>
          <table width=100% id=cariStayPermit name=cariStayPermit class=display><thead>
          <tr><th width=10 >no</th><th width=200 >STATES</th><th  width=400 >PERMANENT MISSIONS</th><th width=100>ACTION</th></tr>
          </thead><tbody>	";
    ...
    // $posisi = $p->cariPosisi($batas);
	  ...
	  $sql= "SELECT DISTINCT `m_negara`.`ID_NEGARA`, `m_negara`.`NEGARA`, `m_kantor_perwakilan`.* FROM (`m_kantor_perwakilan`)
			...
			AND `m_kantor_perwakilan`.`ID_GROUP` = '2' OR `m_kantor_perwakilan`.`ID_GROUP` = '3' ORDER BY `ID_ORDER` asc";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				&nbsp $r[NEGARA] </td><td> <a href=?module=perutusan&act=view&idt=$r[ID_KNT_PERWAKILAN]>$r[NM_KNT_PERWAKILAN]</a> </td>
		            <td align=center><a href=?module=kantor&act=editkantor&idt=$r[ID_KNT_PERWAKILAN]>Edit</a>
		                </td>
		        </tr>";
      $no++;
    }
  ...
  case "tambahperutusan":
    echo "<h2>Tambah Permanent Missions to ASEAN</h2>
  ...
  $sql="SELECT * FROM v_dipsib_list
	...
	while($data = mysql_fetch_array($edit))
	{
		  ...
		  <tr >
					<td wwidth=5%>$no </td>
					<td width=30%> <a href=?module=diplomat&act=viewdiplomat&idt=$data[ID_DIPLOMAT]&negara=$_GET[negara]>$tHS $data[NM_DIPLOMAT]</a> <br> $tSP $nSP</td>
