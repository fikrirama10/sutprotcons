...
//<tr><th width=30>no</th><th width=80>NO PENDAFTARAN</th><th width=80>STATUS</th><th width=80>NO PERMIT</th><th width=40>JNS PERMIT</th><th>NAMA LENGKAP</th><th width=170>KANTOR PERWAKILAN</th><th width=170>JABATAN</th><th width=80>TGL BERLAKU</th><th width=70>AKSI</th></tr>";
   ...
   // if (!empty($neg)){
	...
	if (isset($_GET['namapemohon']) || isset($_GET['indeksvisa']) || isset($_GET['pwk']) || isset($_GET['stat'])){
			...
			$sql="SELECT * FROM tbl_trans_otvis $term_wh $np $iv $pwk $st order by created_date desc limit $posisi,$batas";
	...
	if($tampilbeda != '' && isset($tampilbeda)){
		...
		//  echo "<tr bgcolor='$bgcolor'>";
				...
				/* 	$sql_visa="select * from m_jns_visa where ID_JNS_VISA = 3 OR ID_JNS_VISA = 2 OR 
					...
					$sql_visa="select * from m_jns_visa where ID_JNS_VISA = 2 OR ID_JNS_VISA = 3 OR ID_JNS_VISA = 4 OR ID_JNS_VISA = 5 
					OR ID_JNS_VISA = 6 OR ID_JNS_VISA = 7 OR ID_JNS_VISA = 8 OR ID_JNS_VISA = 9 OR ID_JNS_VISA = 10 OR ID_JNS_VISA = 25 
					OR ID_JNS_VISA = 26 OR ID_JNS_VISA= 27 OR ID_JNS_VISA= 28"; //Tim Dam tambah OR ID_JNS_VISA= 28
					}
					else
					{
					$sql_visa="select * from m_jns_visa where ID_JNS_VISA = 2 OR ID_JNS_VISA = 3 OR ID_JNS_VISA = 4 OR ID_JNS_VISA = 5 
					...
					echo "</td>";
					echo "<td>".date('d-m-Y h:m:s',strtotime($r[created_date]))."</td>
					<td align='center'>";
					...
					echo "
					<td align=center>";
					if ($r['status_permohonan'] == 1 || $r['status_permohonan'] == 2)
							{
...
case "tambah_visa_old":
 ...
 // $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idt  ");
	...
	echo "	
		<tr>
		<tr><td width=120><u>Tanggal</u> <br> <i>Date</i></td>  <td  colspan='2'> : 
		";
		echo date('d-m-Y');
	...
	$tampil=mysql_query("SELECT * FROM m_syarat where jenis_izin='1'");
		...
		<tr><td><u>Paspor</u> &nbsp; <font color='red'>*</font><br><i>Passport</i></td>     <td  colspan='2'> : <input type=text name='paspor_otvis' id='paspor_otvis' size=50  >
 ...
 // $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idt  ");
	...
	$tampil=mysql_query("SELECT * FROM m_syarat where jenis_izin='1'");
	 /*echo "<tr><td>Perwakilan RI &nbsp; <font color='red'>*</font></td>     
		...
		<textarea name='tujuan_otvis' id='tujuan_otvis' rows=2 cols=35 maxlength=500 ></textarea>
		</td></tr>";
 ...
 case "sunting_visa":
	...
	echo" 
		<tr>
		<tr><td  width=120>No Konsep</td>  <td > : $r[no_konsep_pwk]</td>
		<input type='hidden' value='$r[no_konsep_pwk]' name='no_konsep_pwk'>
		<td rowspan='8'><div align=center style='padding: 30px 4px 20px 4px;'><img src=\"../files/otvis/foto/$r[foto]\" width=140 height=180 border=1> </div></td>
				 
		</tr>
		<tr><td  width=120>No Otvis</td>  <td > : ";if($r[no_konsep_pusat]) { echo $r[no_konsep_pusat];} else {echo "&nbsp;<i>belum diproses</i>";} echo "
	...
	if($_SESSION[G_leveluser]!=15)
	{
		if($r[pwk_ri] != null)
		{
	echo "<tr><td>Perwakilan RI &nbsp; <font color='red'>*</font></td>     <td> : <select $disab_link name='pwk_otvis' id='pwk_otvis'>
			<option>- Silahkan Pilih -</option>
			"; 
		...
		<td><u>Nama</u>&nbsp; <font color='red'>*</font><br><i>Name</i> </td>     <td> :"; 
		?>
	...
	echo "<tr><td>Kewarnegaraan &nbsp; <font color='red'>*</font></td>     <td> : 
		...
		$sql_fam="select * from tbl_anggota_fam where id_otvis = '$r[id_otvis]'";
		 ...
		 while($val=mysql_fetch_array($tampil_fam))
			...
			echo "<tr id='rowIdw$h'><td></td><td width='10px;'> $val[urutan]. </td>
				 ...
				 echo" <td rowspan='$rsp'><div align=center style='padding-top: 17px'><img src=\"../files/otvis/foto/$val[foto]\" width=140 height=180 border=1> </div></td>
				  ...
				  <td>: &nbsp; <select class='inputIdw$h' name='anggota_fam[$h][anggotafam_relasi]' id='inputanggotafam_relasi'><option>- Silahkan Pilih -</option>
				  ";
		...
		<textarea name='tujuan_otvis' id='tujuan_otvis' rows=2 cols=35 maxlength=500 $read_link>$r[tujuan]</textarea>
	   ...
	   echo "</td></tr>
		...
		echo "					<tr><td width='120'>Keputusan </td> <td> :
							...
							echo"></select> Lama Tinggal (1-30 Hari) : <input type=number name='lama_tinggal' value = '$r[lama_tinggal]' id='lama_tinggal' min=1 max=30 size=5> Hari
							</td></tr>
							<!-- TIM DAM -->
							<tr><td>Tgl. Keputusan &nbsp;</td><td><DIV id=\"tgl\">";  if (empty($r[tgl_keputusan])) { echo "<script>DateInput('tgl_keputusan', true, 'YYYY-MM-DD')</script>";} else { echo "<script>DateInput('tgl_keputusan', true, 'YYYY-MM-DD','$r[tgl_keputusan]')</script>"; } echo"</div> </td></tr>
							</table> <!-- TIM DAM -->";
							}
							else
							{
							...
							<textarea name='catatan_otvis' id='catatan_otvis' rows=2 cols=35 $read_link1>$r[catatan]</textarea>
							</td></tr>
							";
