...
//end epo manual
    ...
    $sql = ("select a.ID_EPO,a.ID_DIPLOMAT,a.NO_EPO,a.TGL_AWAL_EPO,a.TGL_AKHIR_EPO, b.NM_DIPLOMAT, a.ST_EPO,a.ST_EPO_K,a.ST_EPO_KAS, a.KD_WORKFLOW from epo_diplomat a, diplomat b where a.ID_DIPLOMAT = b.ID_DIPLOMAT and b.ID_DIPLOMAT = $idt and a.KD_WORKFLOW>=1 order by  a.TGL_AKHIR_EPO desc limit 0,1 ");
    ...
    if (($r[ST_EPO_K] == 2) and ( $r[ST_EPO_KAS] == 2) and ( $r[KD_WORKFLOW]) == 3) {
      ...
      <form method=POST enctype="multipart/form-data" action="./report.php?go=epo">
        ...
        <input type="submit" value="Cetak" target="_blank" onclick="location.href = ./report.php?go=epo&idt=<?php echo $idt; ?>">
        <?php if ($jns == 'manual') { ?>
    ...
    echo "
		 <table width=100%>
		   <tr><th  width=30>no</th><th>No Pendaftaran</th><th>Status</th><th >NO EPO</th><th width=70>Tanggal Awal</th><th width=70>Tanggal Akhir</th><!--<th width=30>DIREKTUR</th>--><th width=30>KASUBDIT</th><th width=30>KASIE</th><th width=60>AKSI</th></tr>";
	...
	ST_EPO,ST_EPO_K,ST_EPO_KAS,NO_DAFTAR, STATUS_WORKFLOW, ID_OTVIS  from  v_epo where $wee = $idt and KD_WORKFLOW>=1 order by  ID_EPO ");
...
FROM tbl_trans_otvis a LEFT JOIN tbl_tipe_visa b on a.tipe_visa = b.id LEFT JOIN m_jns_visa c on a.indeks_visa=c.ID_JNS_VISA LEFT JOIN m_negara d on a.kewarganegaraan = d.ID_NEGARA LEFT JOIN tbl_jns_paspor e on a.jns_paspor = e.id LEFT JOIN tbl_perwakilan f on a.pwk_ri=f.id_perwakilan LEFT JOIN m_kantor_perwakilan g on a.tempat_tugas = g.NM_KNT_PERWAKILAN where a.id = $idt";
	...
	<br>";
      ...
      while ($data = mysql_fetch_array($tampil)) {
        echo "$no. $data[syarat_nama]   <input type=hidden name='SYARAT_KD[]' value='$data[syarat_kd]'><input type=file name='syarat_file[]' required><br>";
        $no++;
      }
      echo "</td></tr>";
...
FROM tbl_trans_otvis a LEFT JOIN tbl_tipe_visa b on a.tipe_visa = b.id LEFT JOIN m_jns_visa c on a.indeks_visa=c.ID_JNS_VISA LEFT JOIN m_negara d on a.kewarganegaraan = d.ID_NEGARA LEFT JOIN tbl_jns_paspor e on a.jns_paspor = e.id LEFT JOIN tbl_perwakilan f on a.pwk_ri=f.id_perwakilan LEFT JOIN m_kantor_perwakilan g on a.tempat_tugas = g.NM_KNT_PERWAKILAN where a.id = $idd";
    ...
    $sql = ("select ID_EPO,ID_DIPLOMAT,NO_EPO,TGL_AWAL_EPO,TGL_AKHIR_EPO,TGL_AMBIL_EPO,KET, NO_DAFTAR, NO_SERI_STIKER from v_epo where ID_EPO = $idc ");
    ...
    if (empty($r[NO_EPO])) {
      ...
      //$urutan = mysql_query("select a.NO_EPO from epo_diplomat a, diplomat b where a.id_diplomat=b.id_diplomat and b.ID_JNS_PASPOR=$data[ID_JNS_PASPOR] order by a.ID_EPO DESC LIMIT 1");
      if($jns == 'manual'){
      $urutan = mysql_query("SELECT x.NO_EPO from
				((select SUBSTRING( a.NO_EPO, 13, 5) AS no_urut, 'diplomat' as status, a.NO_EPO from epo_diplomat a where a.TIPE_VISA='" . $data[TIPE_VISA] ."' and a.NO_EPO IS NOT NULL )
				UNION
				(select SUBSTRING( a.NO_EPO, 13, 5) AS no_urut , 'SIBLING' as status, a.NO_EPO from epo_sibling a where a.NO_EPO IS NOT NULL ))
				as x
        ...
        $urutan = mysql_query("SELECT x.NO_EPO from
				((select SUBSTRING( a.NO_EPO, 13, 5) AS no_urut, 'diplomat' as status, a.NO_EPO from epo_diplomat a where a.TIPE_VISA='" . $data[TIPE_VISA] ."' and a.NO_EPO IS NOT NULL )
				UNION
				(select SUBSTRING( a.NO_EPO, 13, 5) AS no_urut , 'SIBLING' as status, a.NO_EPO from epo_sibling a where a.TIPE_VISA='" . $data[TIPE_VISA] ."' and a.NO_EPO IS NOT NULL ))
				as x
				order by x.no_urut DESC LIMIT 1");
      }
      ...
      $nomor = "KAF/" . $sekarang . "/" . $kd_agenda . "/" . $tipe_visa . "/" . $urutnya;
      //print_r($nomor);exit;
      //echo "$kode";exit;
      echo "<tr><td>No EPO</td>     <td> : <input type=text name='NO_EPO' size=50  value=$nomor  ></td></tr>";
    } else {
      echo "<tr><td>No EPO</td>     <td> : <input type=text name='NO_EPO' size=50  value='$r[NO_EPO]' readonly></td></tr>";
    }


    echo "	  <tr><td>No Seri Stiker</td>     <td> : <input type=text name='NO_SERI_STIKER' size=50  value='$r[NO_SERI_STIKER]' required></td></tr>
    ...
    if (empty($r[TGL_AMBIL_EPO])) {
      echo "<script>DateInput('TGL_AMBIL_EPO', true, 'YYYY-MM-DD','" . date("Y-m-d") . "')</script>";
    } else {
