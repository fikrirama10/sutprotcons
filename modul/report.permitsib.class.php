...
class card
{
	...
	$data = mysql_query("select  KETVER,KETHOR,NO_AGENDA,date_format(TGL_AGENDA,'%d %M %Y') as TGL_AGENDA,NO_IZIN_PERMIT,date_format(TGL_AWAL_PERMIT,'%d %M %Y') as  TGL_AWAL_PERMIT,date_format(TGL_AKHIR_PERMIT,'%d %M %Y') as  TGL_AKHIR_PERMIT,KET,NM_JNS_PERMIT,KD_JNS_PERMIT,ID_SIBLING,NM_SIBLING  from v_stay_permit_sib where ID_PERMIT_S = $idpermit ");	
	$this->rdata  = mysql_fetch_array($data);
	$data2 = mysql_query("select NO_PASPOR, NM_KNT_PERWAKILAN from v_sibling where ID_SIBLING = $idsibling");
	$this->rdata2 = mysql_fetch_array($data2);
	$sql="select FOTO, NM_SIBLING from sibling where ID_SIBLING = $idsibling";
	...
	{	
		...
		$txt .=	'
			  ...
			  <tr>
				<td colspan="4" align="center" >
					<u><strong>Izin Tinggal / Izin Keluar Masuk Beberapa Kali Perjalanan</strong></u><br />
					Stay Permit / Multiple Exit Re-Entry Permit
			</td>
			  </tr>
			  <tr><td colspan="4">&nbsp;</td></tr>
			  <tr>
				<td rowspan="5" valign="middle" align="right">
					<img src="../foto sibling/'.$this->rdata3[FOTO].'" border="0" height="60" width="47"/>&nbsp;&nbsp;&nbsp;
				</td>
			  ...
			  <tr>
				<td width="75"><u>Masa Berlaku</u><br />
				  Valid Until</td>
				<td width="15">:</td>
				<td>'.$tgl_format[0].' '.$bln.' '.$tgl_format[2].'</td>
			  </tr>
			  <tr>
				<td>&nbsp;</td>
				<td width="150">&nbsp;</td>
				<td align=right>Jakarta, '.date("j").' '.bulan().' '.date("Y").'</td>
				<td></td>
			  </tr>
			  <tr>
				<td width="47">&nbsp;</td>
				<td width="195">'.$this->rdata[KETHOR].'</td>
				<td align=right>&nbsp;</td>
				<td></td>
			  </tr>
			</table>
		...
		<table width="325">
		<tr>
		<td width="60" align="left"><img src="../foto sibling/'.$this->rdata3[FOTO].'" border="0" height="55" width="42"/></td>
		<td width="205" align="center"> <strong><h3>Kementerian Luar Negeri RI</h3></strong></td>
		<td width="60"></td>
		</tr>
		</table>
		<table width="325">
		...
		<td width="190"> : '. $this->rdata[NM_SIBLING].'</td>
		</tr>
		<tr>
		<td width="25"></td>
