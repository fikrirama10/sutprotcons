...
//(SELECT CONCAT('KAG/',date_format('$TGL_AGENDA','%y'),'/',(SELECT KD_JNS_PASPOR FROM v_sibling WHERE ID_SIBLING = $ID_SIBLING),'/',$urut_permit))
    mysql_query($sql);
 	}else
	{
		$cmp1="SELECT COUNT(ID_PERMIT_S) as jml FROM v_stay_permit_sib WHERE NO_IZIN_PERMIT LIKE CONCAT('KAG/',date_format('2012-12-26','%y'),'/%')";
		...
		$sql="insert into permit_sibling (ID_SIBLING,ID_JNS_PERMIT,NO_AGENDA,TGL_AGENDA,NO_IZIN_PERMIT,TGL_AWAL_PERMIT,TGL_AKHIR_PERMIT,KET,ST_PERMIT,ST_PERMIT_K,ST_PERMIT_KAS,NO_NOTA,TGL_NOTA) values ($ID_SIBLING,$ID_JNS_PERMIT,'$NO_AGENDA','$TGL_AGENDA','$NO_IZIN_PERMIT','$TGL_AWAL_PERMIT','$TGL_AKHIR_PERMIT','$KET','$KETVER','$KETHOR',1,1,1,'$NO_NOTA','$TGL_NOTA')";
//(SELECT CONCAT('KAF/',date_format('$TGL_AGENDA','%y'),'/',(SELECT KD_JNS_PASPOR FROM v_sibling WHERE ID_SIBLING = $ID_SIBLING)
	//   echo $sql; 
	mysql_query($sql);	
	}
...
if  ($TGL_AKHIR_PERMIT > $b[BATAS_PERMIT])
	...
	function sendemail($nodaftar,$nama,$namasibling,$tglawal,$tglakhir,$noizin,$jnspermit,$email,$kd_flow,$ket,$tglambil)
	{
		...
		<html>
				...
				<p>Permohonan atas nama : <strong>$namasibling</strong> dengan nomor registrasi : <strong>$nodaftar</strong> telah disetujui, mohon agar dapat menyerahkan berkas sebagai berikut : <br>
				<br>
				<table>
				<tr>
				<td>- &nbsp;&nbsp;&nbsp;&nbsp;Kedutaan</td>
				...
				<p>Your application with the registration number : <strong>$nodaftar</strong> for <strong>$namasibling</strong> has been verified, kindly submit the original documents as followed : <br>
				<br>
				<table>
				<tr>
				<td>- &nbsp;&nbsp;&nbsp;&nbsp;Embassy</td>
		...
		if($kd_flow == 2)
		{
			...
			$subject = 'Konfirmasi Ijin Tinggal Online No. Daftar : '.$nodaftar.' (SIBLING) Disetujui / Stay Permit Confirmation Reg. No : '.$nodaftar.' (SIBLING) Approved';
		
		}
		elseif($kd_flow == 1)
		{
			$message = $messagetolak;
			$subject = 'Konfirmasi Ijin Tinggal Online No. Daftar : '.$nodaftar.' (SIBLING) Ditolak / Stay Permit Confirmation Reg. No : '.$nodaftar.' (SIBLING) Rejected';
	...
	if($VERIFIKASI == 1)
	{
		//print_r($VERIFIKASI. ' siplah');exit;
		if($c['KD_WORKFLOW'] != 1)
		{
	...
	elseif($VERIFIKASI == 2)
	{
		...
		if($c['KD_WORKFLOW'] != 3)
		{
		//print_r($VERIFIKASI. ' okelah kalo');exit;
		sendemail($c['NO_DAFTAR'],$c['NM_DIPLOMAT'],$c['NM_SIBLING'],$c['TGL_AWAL_PERMIT'],$c['TGL_AKHIR_PERMIT'],$c['NO_IZIN_PERMIT'],$c['NM_JNS_PERMIT'],$c['EMAIL_EMBASSY'],$VERIFIKASI,$KET,$TGL_AMBIL);
		}
	}
