...
<?php
	  ...
	  // echo "<br><a href=?module=staypermit&act=cari&huruf=A>A</A> |	<a href=?module=staypermit&act=cari&huruf=B>B</A> |	<a href=?module=staypermit&act=cari&huruf=C>C</A> |	<a href=?module=staypermit&act=cari&huruf=D>D</A> |	<a href=?module=staypermit&act=cari&huruf=E>E</A> |	<a href=?module=staypermit&act=cari&huruf=F>F</A> |	<a href=?module=staypermit&act=cari&huruf=G>G</A> |	<a href=?module=staypermit&act=cari&huruf=H>H</A> |	<a href=?module=staypermit&act=cari&huruf=I>I</A> |	<a href=?module=staypermit&act=cari&huruf=J>J</A> |	<a href=?module=staypermit&act=cari&huruf=K>K</A> |	<a href=?module=staypermit&act=cari&huruf=L>L</A> |	<a href=?module=staypermit&act=cari&huruf=M>M</A> |	<a href=?module=staypermit&act=cari&huruf=N>N</A> |	<a href=?module=staypermit&act=cari&huruf=O>O</A> |	<a href=?module=staypermit&act=cari&huruf=P>P</A> |	<a href=?module=staypermit&act=cari&huruf=Q>Q</A> |	<a href=?module=staypermit&act=cari&huruf=R>R</A> |	<a href=?module=staypermit&act=cari&huruf=S>S</A> |	<a href=?module=staypermit&act=cari&huruf=T>T</A> |	<a href=?module=staypermit&act=cari&huruf=U>U</A> |	<a href=?module=staypermit&act=cari&huruf=V>V</A> |	<a href=?module=staypermit&act=cari&huruf=W>W</A> |	<a href=?module=staypermit&act=cari&huruf=X>X</A> |	<a href=?module=staypermit&act=cari&huruf=Y>Y</A> |	<a href=?module=staypermit&act=cari&huruf=Z>Z</A>";
		...
		$sql_kewarganegaraan="select * from m_negara order by NEGARA ASC";
		$tampil_kewarganegaraan=mysql_query($sql_kewarganegaraan);
...
switch($_GET[act]){
  default:
		...
		echo "<h2>Daftar Pengajuan Visa</h2>
			...
			<table width='400' border='0px' cellspacing='0'>
			<tr>
...
//<tr><th width=30>no</th><th width=80>NO PENDAFTARAN</th><th width=80>STATUS</th><th width=80>NO PERMIT</th><th width=40>JNS PERMIT</th><th>NAMA LENGKAP</th><th width=170>KANTOR PERWAKILAN</th><th width=170>JABATAN</th><th width=80>TGL BERLAKU</th><th width=70>AKSI</th></tr>";
   ...
   // if (!empty($neg)){
	...
	if($_GET['namapemohon']!= '')
	{
		...
		if($_GET['kewarganegaraan']!= '' )
		{
		 $term_kw = " AND ID_NEGARA = ".$_GET['kewarganegaraan'];
		}

		$np = " nama like '%".$_GET['namapemohon']."%'".$term_pwk.$term_iv.$term_st.$term_kw;
	}
	elseif($_GET['indeksvisa']!= '')
	{
		...
		if($_GET['kewarganegaraan']!= '' )
		{
		 $term_kw = " AND ID_NEGARA = ".$_GET['kewarganegaraan'];
 		 ...
 		 $iv = " $term indeks_visa = ".$_GET['indeksvisa'].$term_pwk.$term_np.$term_pwk_ri.$term_st.$term_kw;
	}
	elseif($_GET['pwk']!= '')
	{
		if($_GET['namapemohon']!= '' && $_GET['stat']!= '')
		...
		$pwk = " $term pwk_ri = ".$_GET['pwk'].$term.$term_np.$term_iv.$term_st.$term_kw;

	}
	elseif($_GET['kewarganegaraan']!= '')
	{
		if($_GET['namapemohon']!= '' && $_GET['stat']!= '')
		{
		 $term = " AND ";
		...
		if($_GET['pwk']!= '' )
		{
		 ...
		 $kw = " $term kewarganegaraan = ".$_GET['kewarganegaraan'].$term_pwk.$term_np.$term_iv.$term_st.$term_kw;
	}
	elseif($_GET['stat']!= '')
	{
		if($_GET['indeksvisa']!= '' && $_GET['stat']!= '')
	...
	if (isset($_GET['namapemohon']) || isset($_GET['indeksvisa']) || isset($_GET['pwk']) || isset($_GET['stat'])  || isset($_GET['kewarganegaraan'])){
		...
		$term_wh = ' where otvis.kewarganegaraan=negara.ID_NEGARA';
			...
			$sql="SELECT
										...
										FROM
										tbl_trans_otvis AS otvis, m_negara as negara $term_wh $np $iv $pwk $st $kw order by status_permohonan desc, created_date desc limit $posisi,$batas";
...
";
	...
	if (isset($_GET[namadiplomat]))
	{
		...
		else
		{
			...
			$v_kw = '&kewarganegaraan='.$_GET['kewarganegaraan'];
			//$np_ri = '&namapemohon='.$_GET['indeksvisa'];
			if($pwk != '' || $np != '' || $iv != '' || $st != '' || $kw != '')
			{
				$term_pusat = ' where tbl_trans_otvis.kewarganegaraan=m_negara.ID_NEGARA AND ';
			}
			$sql ="select * from tbl_trans_otvis, m_negara $term_pusat $np $pwk $iv $st $kw";
   ...
   //$ilink = "?module=indeksvisa&negara=$_GET[negara]&namadiplomat=$_GET[namadiplomat]";
	 $ilink = "?module=indeksvisa".$st_w.$iv_ri.$pwk_ri.$v_kw;
...
case "tambah_visa_old":
 ...
 // $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idt  ");
	...
	echo "<tr><td>Perwakilan RI</td>     <td> : <select name='pwk_otvis'>
		...
		<tr><td>Catatan</td>     <td> :
		<textarea name='catatan_otvis' rows=6 cols=135 ></textarea>
		</td></tr>
	...
	echo "
		...
		<form method=POST enctype='multipart/form-data' action='./aksi_visa_new.php?module=indeksvisa&act=input'>
			...
			<table width=100%>
				<tr>
				<tr>
					<td width=200><u>Tanggal</u> <br> <i>Date</i></td>
					<td  colspan='2'> :";
						echo date('d-m-Y');
				...
				echo"
				<tr>
					<td><u>Nama</u>&nbsp; <font color='red'>*</font><br><i>Name</i></td>
					<td  colspan='2'> : <input type=text  name='nama_otvis' id='nama_otvis' size=50  ></td>
				</tr>
				<tr>
					<td><u>Tanggal Lahir </u>&nbsp; <font color='red'>*</font><br><i>Date of Birth</i></td>
					<td colspan='2'> : <input type='text' name='tgl_lahir_otvis' id='tgl_lahir_otvis' class='TGL_LAHIR' /></td>
				</tr>
				<tr>
					<td><u>Jenis Kelamin </u>&nbsp; <font color='red'>*</font><br><i>Sex</i></td>
					<td colspan='2'> : 
						<select name='jns_kelamin_otvis' id='jns_kelamin_otvis'>
							<option value=''>- Silahkan Pilih -</option>
				...
				<tr>
					<td><u>Kewarnegaraan</u>&nbsp; <font color='red'>*</font><br><i>Nationality</i> </td>
					<td  colspan='2'> :
				...
				<tr>
					<td><u>Foto</u> &nbsp; <font color='red'>*</font><br><i>Photo</i> </td>
					<td > : <input type=file size=40 id='foto_upload' name='foto_upload' onchange='validate_photo(this)' />
					...
					<td>
						<img id='user_img' height='130' width='110' />
					</td>
				</tr>

				<tr>
					<td><u>Paspor</u> &nbsp; <font color='red'>*</font><br><i>Passport</i></td>
					<td> : <input type=text name='paspor_otvis' id='paspor_otvis' size=50  >
						<br><br>&nbsp;&nbsp;<input type=file size=40 id='foto_paspor_upload' name='foto_paspor_upload' onchange='validate_paspor(this)'>
					</td>
					<td>
						<img id='paspor_img' height='130' width='110' />
					</td>
				</tr>
				<tr>
			...
			<table width=100% id='anggotaFamResultList'>
				<tr>
					<td width=200><u>Anggota Keluarga</u> <br><i>Family/Sibling</i>&nbsp; </td>
					<td colspan=3> : <input type='button' name='tambah_anggota_fam' value='+ Add'></td>
				</tr>
			</table>

			<table width=100%>
				<tr>
					<td width=200>Jenis Paspor &nbsp; <font color='red'>*</font></td>
					<td> :
				...
				<tr>
					<td width=200>Profesi pemohon &nbsp; <font color='red'>*</font></td>
					<td> :
						<select name='profesi_otvis' id='profesi_otvis'>
							<option value=''>- Silahkan Pilih -</option>
				...
				<tr>
					<td><u>Tujuan</u> &nbsp; <font color='red'>*</font><br><i>Purpose</i></td>
					<td> :
						<!-- Update Tim DAM-->
						<textarea name='tujuan_otvis' id='tujuan_otvis' rows=3 cols=135 maxlength=500></textarea>
					</td>
				</tr>
				<tr>
					<td><u>Posisi</u> &nbsp; <font color='red'>*</font><br><i>Position</i></td>
					<td> :
						...
						<div id='replacement' style='display:none;'>
							<br>&nbsp;&nbsp;<input type=text placeholder='masukkan nama Pejabatnya' id='pengganti' name='pengganti' size=50>
						</div>
					</td>
				</tr>

				<tr>
					<td><u>Masa Penugasan </u>&nbsp; <font color='red'>*</font><br><i>Assignment Period</i></td>
					<td>
						<div class=\"tgl_awal\" id=\"tgl\">";  
							if (empty($r[masa_awal_tugas])) {
								echo "
								<script>DateInput('masa_awal_tugas', true, 'YYYY-MM-DD','".date("Y-m-d")."')</script>";
							}
							echo "
						</div> s/d 
						<div class=\"tgl_akhir\" id=\"tgl\">";
							if (empty($r[masa_akhir_tugas])) {
								echo "
								<script>DateInput('masa_akhir_tugas', true, 'YYYY-MM-DD','".date("Y-m-d")."')</script>";
							} 
							echo"
						</div>
					</td>
				...
				<tr>
					<td>Surat Setneg &nbsp;  </td>
					<td> : <input type=file size=40 id='setneg_upload' name='setneg_upload' onchange='validate_docdll(this)'>
						<ul>
				...
				<tr>
					<td><u>Tempat Penugasan </u>&nbsp; <font color='red'>*</font><br><i>Assignment Place</i></td>     <td> : 
						<input type=text placeholder='masukkan tempat penugasan di Indonesia' id='tempattugas_otvis' name='tempattugas_otvis' size=50><input type=hidden id='kd_tempattugas_otvis' name='kd_tempattugas_otvis'>
					</td>
				</tr>
				<tr>
					<td><u>Nota Diplomatik</u> &nbsp; <font color='red'>*</font> <br><i>Diplomatic Note</i></td>
					<td> : 
						<input type=text name='no_nota_diplomatik' id='no_nota_diplomatik' size=50 />
						<br><br>&nbsp;&nbsp;
						<input type=file size=40 id='nota_diplomatik_upload' name='nota_diplomatik_upload' onchange='validate_docdll(this)' />
						<ul>
			...
			<table width=100%>
				<tr>
					<td width='200'>Kepala Perwakilan &nbsp; <font color='red'>*</font></td>
					<td> :
						<input type=text name='keppri' value='' id='keppri' size=50 />
					</td>
				</tr>
				<tr>
					<td width='200'>Jabatan Kepala Perwakilan &nbsp; <font color='red'>*</font></td>
					<td> :
						<input type=text name='jbt_keppri' value = '' id='jbt_keppri' size=50 />
					</td>
				</tr>
				<tr>
					<td width='200'>Pejabat Konsuler &nbsp; <font color='red'>*</font></td>
					<td> :
						<input type=text name='pjbt_konsuler' value = '' id='pjbt_konsuler' size=50 />
					</td>
				</tr>
				<tr>
					<td width='200'>Jabatan Pejabat Konsuler &nbsp; <font color='red'>*</font></td>
					<td> :
			...
			echo "
			<div id='popup_box1' style='overflow-y:scroll;' class='popup_box1'>
				...
				<div>
					<table width='100%'>
						...
						<tr>
							<td width='30%'>No Paspor</td>
							<td width='6'>:</td>
							<td>
								<div id='nopaspor_pre'></div>
							</td>
						</tr>
						<tr>
							<td width='30%'>Jenis Paspor</td>
							<td width='6'>:</td>
							<td>
								<div id='jns_paspor_pre'></div>
							</td>
						</tr>
						<tr>
							...
							<td width='6'>:</td>
							<td>
								<div id='tujuan_pre'></div>
							</td>
						</tr>
						<tr>
							<td width='30%'>Indeks Visa</td>
							<td width='6'>:</td>
							<td>
								<div id='indeksvisa_pre'></div>
							</td>
						</tr>
						<tr>
							<td width='30%'>Masa Penugasan</td>
							<td width='6'>:</td>
							<td>
								<div id='masa_tugas_pre'></div>
							</td>
						</tr>
					</table>";
					...
					if($_SESSION['G_leveluser']==14) {
						...
						<table width=100%>
							...
							<tr>
								<td width='30%'>Jabatan Verifikator </td>
								<td> :</td>
								<td>
							...
							<tr>
								<td width='30%'>Jabatan Legalisator </td>
								<td> :</td>
								<td>
					...
					echo"
					<input type='submit'  name='simpan' value='Simpan' onclick='return konfirmasi()'/>
					</form>
				</div>
 ...
 // $input = mysql_query("select ID_DIPLOMAT,ID_NEGARA,ID_KNT_PERWAKILAN,ID_JNS_PASPOR,ID_JNS_VISA,NM_DIPLOMAT,TEMPAT_LAHIR,DATE_FORMAT(TGL_LAHIR,'%d %M %Y') AS TGL_LAHIR,JK,FOTO,PEKERJAAN,ALAMATLN,ST_SIPIL,NO_PASPOR,PASPOR_OLEH,DATE_FORMAT(PASPOR_TGL,'%d %M %Y') AS PASPOR_TGL,DATE_FORMAT(AKHIR_BERLAKU,'%d %M %Y') AS AKHIR_BERLAKU,NO_VISA,VISA_OLEH,LAMA_BERDIAM,DATE_FORMAT(TGL_TIBA,'%d %M %Y') AS TGL_TIBA,ALAMATIN,NO_SETKAB,DATE_FORMAT(BERLAKUSD,'%d-%M-%Y') AS BERLAKUSD,NO_SPONSOR,ST_VISA from diplomat where ID_DIPLOMAT = $idt  ");
	...
	$tampil=mysql_query("SELECT * FROM m_syarat where jenis_izin='1'");
	 /*echo "<tr><td>Perwakilan RI &nbsp; <font color='red'>*</font></td>
		...
		<!--Update Tim DAM-->
		<textarea name='tujuan_otvis' id='tujuan_otvis' rows=3 cols=135 ></textarea>
		</td></tr>";
	   ...
	   <td> :
		...
		<div id='replacement' style='display:none;'><br>&nbsp;&nbsp;<input type=text placeholder='masukkan nama Pejabat' id='pengganti' name='pengganti' size=50>
	   </div>
	   </td></tr>
 ...
 case "sunting_visa":
	...
	echo "<tr><td>Perwakilan RI &nbsp; <font color='red'>*</font></td>     <td colspan='4'> : <select $disab_link name='pwk_otvis' id='pwk_otvis'>
		...
		echo"</td></tr>
		";
		echo "<tr><td  width=120><u>Tanggal Lahir</u> <br> <i>Date of Birth</i></td>  <td colspan='4'> :
		";
		//echo date('d-m-Y',strtotime($r[tgl_lahir]));
		echo "<input type='text' name='tgl_lahir_otvis' id='tgl_lahir_otvis' class='TGL_LAHIR' value='$r[tgl_lahir]'  />";
		echo"
		</td>
		</tr>";

	echo "<tr><td><u>Jenis Kelamin</u> <br> <i>Sex</i></td>     <td colspan='4'> :
		<input type=hidden name='sex2' id='sex2' size=50  value='$r[sex]'>

		<select name='jns_kelamin_otvis' id='jns_kelamin_otvis' $disab_link>
			<option value=''>- Silahkan Pilih -</option>";
			?>
			<option value='MALE' <?php if ($r['sex']=="MALE") echo "selected"?>>Laki-laki</option>
			<option value='FEMALE' <?php if ($r['sex']=="FEMALE") echo "selected"?>>Perempuan</option>
			<?php
		echo "</select>
	...
	echo "<tr><td>Kewarnegaraan &nbsp; <font color='red'>*</font></td>     <td colspan='4'> :
		...
		$h=1;
		 while($val=mysql_fetch_array($tampil_fam))
			{
				$sql_kewarganegaraan="select * from m_negara order by NEGARA ASC";
         		$tampil_wn=mysql_query($sql_kewarganegaraan);	
			...
			echo "<tr id='rowIdw$h'><td></td><td width='10px;'> $val[urutan]. </td>
				...
				if (!empty($val[USIA])){
 					...
 					if ($jenis_relasi == 3){
						...
						$beda_tahun = ($batas_usia-$age)-1;
							...
							elseif($beda_tahun <= 3) {
								...
								$warning = "<div style='font-size:15px;color:green; float:left;' >Usia Anak memenuhi persyaratan pemberian Otorisasi Visa.</div>";
							}
				 ...
				 echo" <td rowspan='$rsp'><div align=center style='padding-top: 17px'><img src=\"../files/otvis/foto/$val[foto]\" width=140 height=180 border=1> </div></td>
					...
					<td>Jenis Kelamin</td>
					<td>
					<select name='anggota_fam[$h][anggotafam_jnskelamin]' id='anggotafam_jnskelamin' $disab_link>
						<option value=''>- Silahkan Pilih -</option>";
						?>
						<option value='MALE' <?php if ($val['sex']=="MALE") echo "selected"?>>Laki-laki</option>
						<option value='FEMALE' <?php if ($val['sex']=="FEMALE") echo "selected"?>>Perempuan</option>
						<?php
					echo "</select>	
					</td>
					</tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type=hidden name='anggota_fam[$h][anggotafam_kewarganegaraan]' id='anggotafam_kewarganegaraan' size=50  value='$val[kewarganegaraan]'>
                                <select  name='anggota_fam[$h][anggotafam_kewarganegaraan]' id='anggota_fam[$h][anggotafam_kewarganegaraan]' $disab_link>
                                    <option>- Silahkan Pilih -</option>
                                    ";
										
									
										while($vals=mysql_fetch_array($tampil_wn))
                                        {
                                            
											
											echo "<option value='$vals[ID_NEGARA]'";
                                            if($val[kewarganegaraan] == $vals[ID_NEGARA])
                                            {
                                                echo 'selected';
                                            }
                                            echo " >$vals[NEGARA]</option>";
                                        }
                                    echo "
                                </select>
                            </td>
				 ...
				 <tr><td></td><td width='10px;'> </td>
				  ...
				  <td>: &nbsp;
				  <textarea id='id_alasantolak_fam' name='anggota_fam[$h][anggotafam_alasantolak]' placeholder='diisi bila keputusan ditolak' rows=6 cols=135>$val[alasan_tolak]</textarea>
				  </td>
				  </tr>
				  </div>
				  ";
				  ...
				  else
				  {
					...
					echo "
					  ...
					  if($r[status_permohonan]=="1" && $_SESSION[G_iduser]=="pustik.pwk") 
					  
					  
					  {
						//echo "&nbsp;&nbsp;&nbsp;<a href='/modul/report.php?go=print_visasib&idt=".$r[id_otvis]."&sib=".$val[id]."' target='_blank'>Cetak Visa</a>";
						 echo "&nbsp;&nbsp;&nbsp;<input type='button' name='button' onClick=loadPopupBox($val[id]); value='Cetak Visa'> ";
									echo "<div id='popup_box$val[id]' class='popup_box' style='height: 120px'>
									<h2>Print Visa Keluarga</h2>
									<div>
									<table width='100%'>
										<tr>
											<td width='40%'>Application No.</td><td width='6'>:</td><td><input type='text' id='txt_appno_$val[id]' name='txt_appno_$val[id]' value='$val[no_app]' maxlength='20'/></td>
										</tr>";
									echo "<tr>
											<td></td>
											<td></td>
											<td><input type='button' name='updateStatus' onclick=printvisa('/modul/report.php?go=print_visasib&idt=".$r[id_otvis]."&sib=".$val[id]."',$val[id]) value='Cetak'></td>
										  </tr>";

									echo "  
										</table>
										</div>
											<a id='popupBoxClose' onClick='unloadPopupBox($val[id]);'>Batal</a>	
										</div>
										";
					  ...
					  <td>: &nbsp;
					  <textarea  placeholder='diisi bila keputusan ditolak' rows=6 cols=135>$val[alasan_tolak]</textarea>
					  </td>
					  </tr>
					  ";
					  }
		...
		echo "</select>
                        ...
                        if ($r[profesi] == 'MILITER') {
                            $militer_selected = ' selected';
                        }
                        if ($r[profesi] == 'POLISI') {
                            $polisi_selected = ' selected';
                        }
                        if ($r[profesi] == 'SIPIL') {
                            $sipil_selected = ' selected';
                        }
                        echo "
                        <tr>
                            <td width=200>Profesi pemohon &nbsp; <font color='red'>*</font></td>
                            <td> :
                                <input type=hidden name='profesi_otvis_old' id='profesi_otvis_old' size=50  value='$r[profesi]'>
                                <select name='profesi_otvis_update' id='profesi_otvis_update' $disab_link>
                                    <option value=''>- Silahkan Pilih -</option>
                                    <option value='MILITER' $militer_selected>MILITER</option>
                                    <option value='POLISI' $polisi_selected>POLISI</option>
                                    <option value='SIPIL' $sipil_selected>SIPIL</option>
                                </select>
                            </td>
                        </tr>
		<tr><td><u>Tujuan</u> &nbsp; <font color='red'>*</font><br><i>Purpose</i></td>     <td> :

		<textarea name='tujuan_otvis' id='tujuan_otvis' rows=3 cols=135 $read_link>$r[tujuan]</textarea>
	   ...
	   <td> :
		...
		if($r[posisi] == 1) echo "display:none"; echo "'><br>&nbsp;&nbsp;<input type=text placeholder='masukkan nama Pejabat' value='$r[pengganti]' id='pengganti' name='pengganti' size=50 $read_link>
	   </div>";
		 }
	   echo "</td></tr>
			...
			";
				 ...
				 while ($val = mysql_fetch_array($tampil_visa)) {
                        ...
                        if ($val[ID_JNS_VISA] == 28) {
                            echo " id='visamultiple'>Multiple</option>";
                        } elseif (
                            $val[ID_JNS_VISA] == 2 || 
                            $val[ID_JNS_VISA] == 3 ||
                            $val[ID_JNS_VISA] == 4 ||
                            ...
                            echo " id='visadiplomatik'>$val[NM_JNS_VISA] Single</option>";
                        } else {
                            echo " id='visadinas'>$val[NM_JNS_VISA] Single</option>";
                        }
                    }

			echo"</select>
                        	Masa Berlaku Visa (default 90 Hari) : <input type=number name='masa_berlaku_visa_otvis' value = '$r[masa_berlaku_visa]' id='masa_berlaku_visa_otvis' min=1 max=365 size=5> Hari. (Biarkan kosong jika masa berlaku default 90 Hari)
			</td></tr>";
		}
		echo "					<tr><td width='120'>Keputusan </td> <td> :
							...
							echo"></select>  Lama Tinggal (1-60 Hari) : <input type=number name='lama_tinggal' value = '$r[lama_tinggal]' id='lama_tinggal' min=1 max=365 size=5> Hari. (untuk Visa Multiple maksimal 365 Hari) <!-- TIM DAM --> </td></tr>
                                                        <!-- TIM DAM -->
							...
							else
							{
								...
								$tampil_status=mysql_query($sql_status);
								 ...
								 if($r[status_permohonan]=="1" && $_SESSION[G_iduser]=="pustik.pwk") 
								 {
									 ...
									 echo "&nbsp;&nbsp;&nbsp;<input type='button' name='button' onClick=loadPopupBox($r[id]); value='Cetak Visa'> ";
									echo "<div id='popup_box$r[id]' class='popup_box' style='height: 120px'>
									<h2>Print Visa </h2>
									<div>
									<table width='100%'>
										<tr>
											<td width='40%'>Application No.</td><td width='6'>:</td><td><input type='text' id='txt_appno_$r[id]' name='txt_appno_$r[id]' value='$r[no_app]' maxlength='20'/></td>
										</tr>";
									echo "<tr>
											<td></td>
											<td></td>
											<td><input type='button' name='updateStatus' onclick=printvisa('/modul/report.php?go=print_visa&idt=".$r[id_otvis]."',$r[id]) value='Cetak'></td>
										  </tr>";

									echo "  
										</table>
										</div>
											<a id='popupBoxClose' onClick='unloadPopupBox($r[id]);'>Batal</a>	
										</div>
										";
							...
							<textarea name='catatan_pwk' id='catatan_pwk' rows=6 cols=135 $read_link >$r[catatan_pwk]</textarea>
							</td></tr>";
							}
							echo"
							<tr><td width='120'>Catatan Pusat</td>  <td> :
							<textarea name='catatan_otvis' id='catatan_otvis' rows=6 cols=135 maxlength=1000 $read_link1 >$r[catatan]</textarea>
							</td></tr>
							";
...
function printvisa(url, i)
{
	//alert(url+"&app_no="+document.getElementById("txt_appno_"+i).value);
	window.open(url+"&app_no="+document.getElementById("txt_appno_"+i).value, "_blank");
