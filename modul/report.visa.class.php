...
class visa
{
	...
	public function visa($idotvis, $app_no){
		

	// $data = mysql_query("select * from tbl_trans_otvis where id_otvis =".$idotvis);
	
	// $this->rdata  = mysql_fetch_array($data);
	$sql_update = "update tbl_trans_otvis set no_app='$app_no' where id_otvis =$idotvis";
	...
	$sql = "select tbl_trans_otvis.no_konsep_pusat, tbl_trans_otvis.nama, tbl_trans_otvis.paspor, 
						...
						tbl_trans_otvis.pejabat_pwk, tbl_trans_otvis.jabatan_pejabat_pwk, tbl_trans_otvis.foto, tbl_trans_otvis.tgl_lahir, tbl_trans_otvis.sex, tbl_trans_otvis.no_app,
	...
	public function formattingprint()
	{
		...
		if ($this->rdata[tgl_lahir])
			$tgl_lahir = strtoupper(date("d-M-Y",strtotime($this->rdata[tgl_lahir])));



		if ($this->rdata[sex])
			$sex = substr($this->rdata[sex],0,1);
		...
		$txt='
			  ...
			  <tr>
				<td rowspan="11" valign="middle" align="right" width="106.5" height="100">
					<br/><img src="../files/otvis/foto/'.$this->rdata[foto].'" border="0" height="71" width="58"/>&nbsp;&nbsp;&nbsp;
				</td>
				<td width="250"><br/>Surname / Given Name<br />
				  '.strtoupper($this->rdata[nama]).'</td>
				
			  </tr>
			  <tr>
				<td width="65" >Passport no. <br />
				  '.strtoupper($this->rdata[paspor]).'</td>
				<td width="30" >Sex <br />
				  '.$sex.' &nbsp;</td>
			  ...
			  <tr>
				...
				'.strtoupper($this->rdata[no_app]).'&nbsp;</td>
