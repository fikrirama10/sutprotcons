...
if ($module=='rantorIbeli' AND $act=='input'){
...
mysql_query("insert into penggunaan_fasilitas(ID_JNS_FASILITAS,ID_DIPLOMAT,ID_KNT_PERWAKILAN,DESKRIPSI,QTY,ST_PERSETUJUAN,MEREK,TAHUN,NO_POLISI,NO_MESIN,NO_IZIN_BELI,TGL_PERSETUJUAN) values (2,$ID_DIPLOMAT,$ID_KNT_PERWAKILAN,'$DESKRIPSI',1,$ST_PERSETUJUAN,'$MEREK','$TAHUN','$NO_POLISI','$NO_MESIN','$NO_IZIN_BELI','$TGL_PERSETUJUAN')");
	...
	header('location: ./deplu.php?module='.$module.'&act=lihat_rantorIbeli&idt='.$ID_DIPLOMAT.'&negara='.$neg);
 


}
elseif ($module=='rantorIbeli' AND $act=='hapus' AND isset($_GET[idt])){
	$idd = $_GET[idd];
  mysql_query("DELETE FROM penggunaan_fasilitas WHERE ID_PENGGUNAA_FAS  ='$_GET[idt]'");
header('location: ./deplu.php?module='.$module.'&act=lihat_rantorIbeli&idt='.$idd.'&negara='.$neg);
 

}

elseif ($module=='rantorIbeli' AND $act=='update' AND isset($_GET[idt])){
$idt = $_GET[idt];
